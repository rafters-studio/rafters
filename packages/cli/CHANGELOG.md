...
- fix(registry): `loadComponent` now discovers and serves **subsystem files** -- the transitive closure of relative imports that resolve inside the component directory but are not framework variants or shared suffixes (#2170). `loadComponent('editor').files` includes `components/ui/editor/editor-history.ts` and all six `components/ui/editor/ops/*.ts`, in addition to the three flat editor files. Subsystem files install nested under `components/ui/<name>/` in the consumer, so `rafters add editor` no longer leaves 19 dangling imports. **`extractSiblingImports`** is joined by `extractAllRelativeImports`, which returns nested paths (`./ops/content`) that the original deliberately filtered out; a new `resolveRelativeImport` helper resolves each specifier to `.ts`, `.tsx`, or `/index.ts` on disk, and a specifier that resolves to nothing throws from `loadComponent` naming the importer and the specifier (the silent-drop that shipped this bug). **`transformFileContent`** rewrites `./editor-history` and `./ops/content` from a main component file to `@/components/ui/editor/editor-history` and `@/components/ui/editor/ops/content` (the nested install path), but leaves `./` imports relative inside a subsystem file (they install in the same subtree). The primitives regex now handles any depth of `../` before `primitives/`, so `ops/types.ts`'s `../../../primitives/types` rewrites to `@/lib/primitives/types`. **`selectFilesForFramework`** treats subsystem `.ts` files as framework-agnostic (they always install, regardless of target). The `ownBasenames` filter extends to subsystem file basenames so none leaks into `primitives`. Flat components (card, typography, container) are byte-identical.
