...
- fix(mcp): `presence` is now measured from disk instead of read from `config.installed`. That field is a record `rafters add` maintains, and a record drifts from the thing it records in both directions: a part pulled in as a dependency never appears in it (`card-action` is on disk in every project that installed `card`, and reported `available`), and an entry survives a manual delete. Presence is the join between the public catalog and the project, so it is measured now -- a `readdir` of the resolved `componentsPath` / `primitivesPath` / `compositesPath`, with an id taken as the basename before the first dot so every file a component ships (`input.tsx`, `input.behavior.ts`, `input.classes.ts`) maps to one id and any of them proves it is there. Target-agnostic, so no extension table has to be kept in sync as targets are added. The scan is deliberately NOT cached while the catalog behind it is: the catalog is a network fetch plus a graph build, the scan is a directory listing, and caching the cheap half is exactly what would have made `rafters add` invisible until an MCP restart. Re-reading per query means the next call sees the new file with no invalidation, no IPC, and no watcher -- `add` writes a file, and the file is the message. The MCP stays read-only. Reported by shingle.
- fix(mcp): a malformed path field in `config.rafters.json` now returns a structured error naming the field instead of throwing. `readConfig` is a raw `JSON.parse`, so a `componentsPath` that is a number reached `resolveReadSet` and crashed the call; the same `configError` recovery shape that previously covered a malformed `installed` block now covers the path fields that replaced it as the presence source.
- fix(mcp): `rafters_generate` now reports whether the component it returns is actually installed. The response gains `presence` (`installed` | `available`) on EVERY call, plus an `install` command (`rafters add <name>`) when it is `available`. Previously generate returned `{ component, target, snippet, slots }` and omitted the one field that decides whether the snippet is usable: it would hand back `<Dialog>...</Dialog>` for a workspace with no `dialog` installed, the agent would paste it, and the build would die on a missing import with nothing in the payload to have warned it. generate resolves through `describe(id, graph, target)` directly rather than `describeWithOverlay`, so it never saw the overlay's presence stamp -- meaning `describe` and `generate` could tell one workspace two different stories about the same component. `presence` is now sourced from the same installed set the overlay stamps describe with, so the two agree by construction. It rides on every response rather than only the `available` case: a field that appears sometimes teaches a caller to stop looking for it. generate still always returns the snippet -- it is the authority on the artifact, not a gatekeeper, and withholding correct code over a missing install step would make it useless to the agent it exists to serve. Reported by shingle's 0.2.3 eval.
- fix(mcp): the `rafters_describe` and `rafters_generate` tool descriptions no longer advertise capabilities that are not there. Both promised a natural-language door that was deliberately descoped for v1, so agents burned calls on a route that returns no match -- including on the contract's own verbatim example. `describe` additionally documented a `.vocab` address form that does not resolve; drilling to the prop already returns the values inline, so the line was stale documentation rather than a missing resolver. Because `plugin/hooks/agent-contract.md` is generated from the live `TOOL_DEFINITIONS`, correcting the descriptions corrects the injected session-start contract in the same build.
- fix(mcp): the `tools.ts` header comment and the `rafters_describe` / `rafters_generate` descriptions in `TOOL_DEFINITIONS` now describe the natural-language intent door as it actually ships, correcting the previous entry above. The door is WIRED -- `handleDescribe` routes a natural-language address through it directly and `handleGenerate` falls back to it at tier (c) -- and the prior fix overcorrected the defect it was fixing: it dropped the NL sentence from `rafters_describe`'s description entirely rather than correcting it, and left `rafters_generate`'s description silent on the door altogether. Both descriptions, and the header comment, now say plainly that the door exists, is deterministic keyword matching over a small curated tag axis, refuses below its match threshold with a note pointing the caller at `describe(components)` rather than guessing, and that #2166 (open) is what will make it score against the intel the tool actually owns. Reported by shingle's 0.2.3 eval.
- fix(mcp): `rafters_generate`'s and `rafters_describe`'s tool descriptions no longer claim things the code does not do, and one gap between them is closed. `generate`'s description said "if slots is empty, the component takes no children -- do not wrap it with content," but `componentService.ts` builds every react facet with `slots: []` unconditionally (react facets carry no slots at all) while still snippeting content-wrapped usage (`<Button>Save</Button>`) -- the sentence told an agent not to wrap react components with children, which is exactly wrong for the primary target. It is removed rather than reworded, since "takes no children" needs real `children`-prop detection to say truthfully, and that is a separate issue. `describe`'s description claimed "every node carries parent and siblings," but `graph.ts` only sets either field on a non-root node with siblings -- a root node (the example the same sentence uses two clauses earlier) carries neither; it now says a part node carries `parent`, plus `siblings` when its parent has other parts, and that children are typed pointers keyed by the prop's own type for props, `part` for sub-components, and `edge` for `composesWith` (the third child type the old sentence omitted). Separately, `generate`'s description promises its refusal comes "with a note pointing you at describe(components)," which was true for `describe` but not for `generate`: tier (c) discarded `matchIntent`'s `IntentNoMatch.note` and tier (d) returned a flat message. `handleGenerate` now carries that note onto the refusal when the intent door ran, so the claim holds for both tools. `plugin/hooks/agent-contract.md` and the plugin bundle are regenerated in the same commit since both are built from these descriptions.
- fix(mcp): `buildInstalledSet` is removed along with its test -- it built an `InstalledSet` from `config.installed`, the exact mechanism the disk-scan fix (above) replaced, and had acquired zero production callers (`scanInstalled` is now the only producer). Keeping it as dead code documented a source of presence this codebase no longer uses; `InstalledSet`'s doc comment is corrected to name `scanInstalled` / the on-disk scan instead.
- fix(mcp): restores `configError` coverage for `rafters_generate` and the deprecated `rafters_component` alias that the presence-from-disk test rewrite dropped. `overlayContext`'s malformed-config guard has three call sites (`handleDescribe`, `handleGenerate`, and the shared `describeById` behind `rafters_component`/`rafters_composite`); only the `handleDescribe` one had a test naming a malformed `componentsPath`, leaving two of the three guarded paths unverified.
- fix(plugin): the agent contract no longer instructs agents to do something the pre-edit hook denies. It said "use semantic color tokens (bg-primary, text-accent)" while `pre-edit.sh` rejects exactly those utilities in assembly code; the contract now points at the composite block's variant meta or the component's `variant` prop, which is where the color decision actually lives. The hook's own denial message was also naming three deprecated MCP tools (`rafters_pattern` / `rafters_composite` / `rafters_component`) and now names `rafters_describe`.
- fix(mcp): the disk-scan presence fix (above) fell back to `.rafters/composites` for `compositesPath` when a workspace has no config at all -- a path no framework's default actually uses (`FRAMEWORK_SPECS.next` writes bare `composites`; other frameworks write `src/composites` or `app/composites`). `scanInstalled` now takes its fallback folders from `reconcile.ts`'s `KIND_PATHS` (exported for this reuse), the same map `--update-all`'s disk reconciliation already reads and the same defaults `rafters init` writes into a fresh config, instead of a second, independently maintained copy of those literals that had drifted from it.
- fix(mcp): `rafters_describe`'s tool description no longer claims `describe(button.props.fill)` "returns the real token values." `fill` is the canonical grammar prop, and a grammar prop's `vocab` (the token layer) is deliberately stripped before it ever reaches the agent surface (`AgentGrammarProp = Omit<..., 'vocab'>`, `graph.ts`); drilling into it returns composition rules, never inlined values. `fill` was also a poor address to demonstrate anything on `button` with: the registry's `extractFacet` (`componentService.ts`) emits only `enum` props, so `button`'s extracted facet carries no `fill` -- the same phantom address doubled as the description's `.?` safe-probe example, and a reader could conclude `button` carries a `fill` prop from either clause. The description now drills a real, tested prop instead (`describe(button.props.variant)`) and says what a drill actually returns: an enum prop's literal union members, or a grammar prop's composition rules with the token vocabulary deliberately withheld; the probe example moves to `describe(button.props.color.?)`, the exact miss case `graph.test.ts` pins to `null`. `plugin/hooks/agent-contract.md` and the plugin bundle are regenerated in the same commit since both are built from this description.
### Breaking changes
