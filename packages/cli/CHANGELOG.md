...
- feat(design-tokens): a new `@utility stagger-items` emits a per-position stagger ladder so a component's `.classes.ts` SELECTS a ready-made class instead of constructing `calc()` / `:nth-child` itself (#2189). `generateStaggerUtility` gives child positions 1 through 12 their own `:nth-child(n)` rule, each declaring `animation-delay: calc(<n> * var(--rafters-delay-stagger-step))`; position 13 and beyond match only the `& > *` fallback rule, which carries the SAME multiplier as position 12 -- the delay saturates rather than resetting to 0 or climbing unbounded. 12 is the cap because it is the size of the largest items/enter collection in `motion.jsonl`'s six stagger-step assignments (dropdown-menu, context-menu, select, combobox, command, menubar), documented explicitly in the exporter so a future matrix entry that exceeds it is a visible decision, not a silent truncation. No literal duration appears anywhere in the block -- every declaration multiplies the designer-owned `--rafters-delay-stagger-step` token by position via `calc()`, so the whole utility is byte-identical across any retune (the toy-9 invariant `generateMotionNamespaceUtilities` already documents). Reduced motion routes through the exporter's existing `REDUCED_MOTION_ZEROED` mechanism, zeroing `animation-delay` under `prefers-reduced-motion: reduce`. Unblocks #2156, which was reverted in PR #2185 for exactly this gap: constructing `calc()`/`:nth-child` directly in `.classes.ts` violates `packages/ui/docs/spec/00-boundaries.md` Sec 6 (classes.ts owns selection among literal strings, never construction). This issue introduces no new tunable value of its own (Rule 0) -- it only multiplies the pre-existing, already-`0ms`-by-default `--rafters-delay-stagger-step` token by position.
- feat(ui): dropdown-menu, context-menu, select, combobox, and command now SELECT `stagger-items` (#2189) on their item collection's container, so `items / enter`'s `stagger-step` cell in `motion.jsonl` compiles to a real per-position `animation-delay` instead of the flat `transition-delay` the generic `delay-stagger-step` token alone produces (#2156). The container is whichever element is the item children's direct DOM parent per component: dropdown-menu, context-menu, and combobox add it to `content`; select adds it to `viewport` (content wraps viewport, which wraps the items); command adds it to `list`. No `.classes.ts` file constructs `calc()` or `:nth-child` itself -- each just appends the literal `stagger-items` string, per `packages/ui/docs/spec/00-boundaries.md` Sec 6 -- and no JavaScript computes or assigns a per-item stagger index. `--rafters-delay-stagger-step` still defaults to `0ms`, so this change is behaviorally invisible until a designer tunes the token; a new probe (`packages/ui/test/motion/stagger-items-consumption.test.ts`) compiles this package's real `src/components` through the real `@tailwindcss/cli` and proves position 3 resolves to `calc(3 * var(--rafters-delay-stagger-step))` with multipliers strictly increasing 1..5 across item positions. Menubar's `items / enter` cell is also `stagger-step` in the matrix but has no component directory under `packages/ui/src/components/` -- it is skipped here and deferred to the matrix-hygiene issue (#2158), which decides whether menubar's cells stay as a needs-ledger entry or are removed.
