...
- feat(init): single-color `@theme` declarations promote to seed families. Previously `@theme { --color-brand: #FF5500 }` (a one-off brand color not part of a ramp) was dropped to leftover and never imported. Now `seedFamiliesFromDeclarations` (new export from `@rafters/design-tokens`) walks color-valued declarations that did not form a ramp, parses each value via `tryParseColor`, generates a full 11-position scale via `generateOKLCHScale`, and preserves the source value at its declared position (defaulting to 500 when the name has no position suffix). Seeds skip when their family base name collides with an existing ramp family. Seed-family names follow the same source-preserving convention as ramps: `--color-brand` -> family `brand`, `--color-brand-700` -> family `brand` with seed at position 700. Verified against huttspawn: single-color discipline/quality/style/combat declarations (28 additional) now promote alongside the 8 ramp families, bringing the full taxonomy into the registry as proper families.
