...
- fix(cli): both regeneration paths -- `init --rebuild` and the post-install regen in `add` -- regenerate the motion namespace instead of reloading the stored one, so they work on a project whose tokens predate 0.3.0 (#2208, and the `add` face filed as #2200). Every pre-0.3.0 project failed, in one of two places: a 0.2.3-era `motion-cell-*` value carries a bare `durationTier` where the current shape is the `{"kind":"tier"|"period"}` union #2154 introduced, and the Tailwind exporter refused it with `motion cell token "motion-cell-dialog-content-open" has an unrecognized duration.kind null`; a spring-2026 `motion.rafters.json` carries `motion-easing-ease-in`, a name since retired from the easing enum, and the load never got as far as the exporter -- `Token "motion-easing-ease-in" failed TokenSchema validation`. Neither is recoverable by reading the file, because motion is system-generated end to end: cells, easings, durations and the five namespaces all come out of `motion.jsonl` through the motion generator, and nobody hand-authors one. A shared helper (`packages/cli/src/utils/motion-rebuild.ts`) therefore rewrites `.rafters/tokens/motion.rafters.json` from `generateNamespaces(['motion'])` -- the same generator call a fresh `init` makes, so a rebuilt namespace holds exactly the tokens a fresh one does -- before the registry load, which then reads the regenerated file with no further surgery, and `regenerateFromExisting` and `regenerateAfterInstall` both call it. Names the generator no longer emits do not come back. This is the treatment `elevation` and `fill` already get (#1638, #1637), except motion is a live namespace, so it is rewritten rather than deleted. The one thing on a motion token that is the designer's and not the generator's is a `userOverride`, and that is carried forward by name onto a token that still exists -- together with the value it documents, since `TokenRegistry` seeds an overridden token from its on-disk value rather than re-deriving it, so carrying the record alone would leave the token claiming a decision it no longer applies. THE CARRY IS GATED ON THE STORED VALUE STILL BEING A SHAPE THIS VERSION EMITS, which is the precondition that pair was missing: a 0.2.3 cell value is still a plain string to `TokenSchema`, so an override on such a cell would write the stale spec straight back over the correctly regenerated one and hand the exporter the identical `duration.kind null` this fix exists to remove. A `motion-cell-*` value that parses as a JSON object must satisfy the current cell spec or BOTH halves are dropped and the token stands exactly as the generator emitted it, `userOverride: null` -- a token holding the new value while still claiming the old decision would assert a choice over a shape that no longer exists. A cell value that is not a JSON object is an operator pin (an animation shorthand the exporter emits verbatim) and still carries, as does every non-cell motion token. The drop is logged as `motion:override-dropped` rather than done silently, and it is surfaced to a person, not only to an agent: `ui.ts` renders it as a `Warning:` line and stops the running spinner first, so the notice is not overwritten by the success line the regeneration step prints immediately after. The stored file is read token by token, never as a whole: the premise is that most of it no longer parses, so a token that fails `TokenSchema` is skipped instead of aborting the read. On the `add` path, "project not initialized" is now decided by the presence of a stored namespace file rather than by a bare catch around the load, so a `TokenParseError` or a corrupt token file reaches `add:regen-failed` instead of being swallowed under a comment about an uninitialized project -- which is why the stale-motion failure left no trace at all there. `init --reset` is untouched -- it already regenerated every system-owned namespace from scratch and takes no path through the rebuild logic.
