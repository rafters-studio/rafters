...
- feat(plugin): the `rafters-frontend` skill is rewritten around why the rules hold rather than a list of prohibitions. The previous opening ("The system makes design choices. You assemble.") was a demotion, and demotions do not survive contact with an agent's training priors. The skill now leads with the reflex correction -- this is not Tailwind and not shadcn -- and the distinction the whole system rests on: the designer's judgment is *encoded* in token values and *applied* in components and composites, so reaching for a pattern carries the judgment with it while reaching past it to a raw value (`bg-primary`, `text-4xl`, `gap-4`) yields a value with the judgment stripped off. That collapses the Never table from thirteen prohibitions into one rule with thirteen faces, and each row now names what it drops. It also explains the `src/components/` exemption, which previously read as an inconsistency: there you are authoring a pattern rather than consuming one, which is the one legitimate place to touch raw tokens. `attentionBudget` is promoted from a config bullet to the agent's primary instrument, since the work the skill frees an agent to do is cognitive -- whether a screen asks the reader to hold too much, whether reading order and priority order agree -- rather than a visual language an agent that cannot see will never speak. The skill's description now also declares that it supersedes generic frontend-design guidance inside a Rafters project, because Anthropic's `frontend-design` skill fires on the same trigger with the opposite instruction. `allowed-tools` is removed: it listed only file and shell tools while the skill's central instruction is to query the MCP, and omitting the field matches how `frontend-design` itself ships.
- build(cli): the version is single-sourced from `packages/cli/package.json` by a new `scripts/sync-version.ts`, run as a step of the CLI `build` alongside the agent-contract generator. npm requires a literal `version` there, making it the one field that cannot be derived and therefore the source of truth; the other five sites (`plugin/package.json`, `plugin/.claude-plugin/plugin.json`, both `version` fields in `.claude-plugin/marketplace.json`, and the `rafters-frontend` SKILL.md frontmatter) are written from it, so building rewrites every site and drift is structurally impossible rather than merely discouraged. Writes are textual rather than parse-then-stringify: re-emitting the JSON reformatted unrelated lines (`"args": ["mcp"]` expanded across three), producing noise diffs and failing `oxfmt`, so each file is parsed only to validate and then edited in place. Each target declares how many `version` fields it is expected to carry, so a manifest gaining or losing a version site fails loudly instead of silently leaving one stale -- which would otherwise reintroduce the exact drift the script exists to remove.
