...
- feat(init): detect remaining base values from source CSS pre-generation. Three new detectors finish the pattern from #1542 / #1544: `detectFontSizeBase` reads `--text-base` (Tailwind v4) or `--font-size-base` (rafters internal) into `baseFontSizeOverride`; `detectFocusRingWidth` reads `--ring-width` or `--focus-ring-width` into `focusRingWidthOverride`; `detectMotionDurationBase` reads `--duration-base` or `--motion-duration-base` (ms / s units) into `baseTransitionDurationOverride`. All five base detectors (spacing, radius, font-size, focus-ring, motion-duration) consolidated behind a shared `detectBase(css, names, parse)` helper in `importers/bases.ts`; `importers/spacing.ts` and `importers/radius.ts` deleted (public exports unchanged). New `init:import_font_size_applied` / `init:import_focus_ring_applied` / `init:import_motion_duration_applied` events fire when applied. Fixes a latent typography-generator bug surfaced by font-size override: per-scale font-size tokens used `size / baseFontSize` (relative to base) instead of `size / 16` (absolute rem). With the default `baseFontSizeOverride: 16` the formulas coincide; with any other override the per-scale values were wrong AND `font-size-base` was duplicated in the output. Changed to `size / 16` -- backward compatible at default base. Closes #1545.
