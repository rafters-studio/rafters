...
- feat(registry): facet props come from the TypeScript type checker, not naming-convention regexes (#2165). `describe()` and the MCP graph now surface real props for components that previously returned `props: {}` or missed individual props. Badge, container, grid, input, card, sidebar, and select all gain their full prop surface. A shared `ts.Program` (one per registry build, not one per component) resolves through alias depth (`(typeof X)[number]`), intersection types (button's `ButtonBaseProps & ...`), and body-level destructuring (grid's `const { preset } = props`). New `PropField` arms -- `boolean`, `string`, `number` -- carry the non-enum props the regex extractor could not represent (container's `query: boolean`, input's `value: string`, container's `colSpan: number`). The react target uses the type checker; astro/vue/svelte keep the interface-body regex (`.astro`/`.vue`/`.svelte` are not valid TypeScript). Defaults now resolve from the AST (boolean `true`/`false`, numeric literals) rather than a string-only regex. The `extractDestructuredNames` function is removed; `extractLiteralUnions`, `extractDestructuredDefaults`, `extractInterfaceProps`, and `inlineUnionValues` remain for non-react targets.
