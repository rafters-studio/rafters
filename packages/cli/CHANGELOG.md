...
- fix(ui): `.classes.ts` files no longer choose a timing value in TypeScript -- every stock Tailwind duration literal (`duration-150`, `duration-200`, `duration-100`, `duration-300`) across ~30 component class sites is replaced with the matrix-assigned tier utility read off its cell in `packages/ui/docs/spec/matrix/motion.jsonl` (#2153). Most hover/colour sites map `150`/`100` -> `duration-fast` (breadcrumb, button, collapsible trigger, dialog/drawer/sheet/popover close, item, nav-menu trigger, pagination, scroll-area scrollbar, sidebar x4, slider thumb, tooltip, select trigger). `tabs` is a real, visible timing correction: its indicator/panel active-change cells (`motion.jsonl:6-7`) assign tier `fast`, not the `duration-200` (`moderate`-equivalent) it shipped with. Three sites land SLOWER than their old literal because their governing cell is the checked/pressed-state colour swap, not the undocumented hover they also carry: `checkbox`, `radio-group`, and `toggle` move `150` -> `duration-moderate`. Four sites land FASTER: `input`, `input-group`, `textarea`, and `input-otp` move their focus-ring `100`/`150` -> `duration-micro`, each per its own cell (not assumed uniform with the other duration-100 sites -- `select`'s trigger cell is `fast`). `switch`, `select` chevron, `toggle-group`, `collapsible` content, and `progress` fill move to `duration-moderate`. Four sites have no matching matrix cell and are deliberately left on their stock literal, deferred to the matrix-hygiene issue (#2158): `badge` (no `badge` component in the matrix at all) and three `navigation-menu` sites (`link`, `chevron`, `indicator` -- only `trigger/hover` and the `panel` cells exist for that component).
