...
- fix(cli): `init --rebuild` regenerates the motion namespace instead of reloading the stored one, so it works on a project whose tokens predate 0.3.0 (#2208). Every pre-0.3.0 project failed the rebuild, in one of two places: a 0.2.3-era `motion-cell-*` value carries a bare `durationTier` where the current shape is the `{"kind":"tier"|"period"}` union #2154 introduced, and the Tailwind exporter refused it with `motion cell token "motion-cell-dialog-content-open" has an unrecognized duration.kind null`; a spring-2026 `motion.rafters.json` carries `motion-easing-ease-in`, a name since retired from the easing enum, and the load never got as far as the exporter -- `Token "motion-easing-ease-in" failed TokenSchema validation`. Neither is recoverable by reading the file, because motion is system-generated end to end: cells, easings, durations and the five namespaces all come out of `motion.jsonl` through the motion generator, and nobody hand-authors one. `regenerateFromExisting` therefore rewrites `.rafters/tokens/motion.rafters.json` from `generateNamespaces(['motion'])` -- the same generator call a fresh `init` makes, so a rebuilt namespace holds exactly the tokens a fresh one does -- before the registry load, which then reads the regenerated file with no further surgery. Names the generator no longer emits do not come back. This is the treatment `elevation` and `fill` already get a few lines above (#1638, #1637), except motion is a live namespace, so it is rewritten rather than deleted. The one thing on a motion token that is the designer's and not the generator's is a `userOverride`, and that is carried forward by name onto a token that still exists -- together with the value it documents, since `TokenRegistry` seeds an overridden token from its on-disk value rather than re-deriving it, so carrying the record alone would leave the token claiming a decision it no longer applies. The stored file is read token by token, never as a whole: the premise is that most of it no longer parses, so a token that fails `TokenSchema` is skipped instead of aborting the read. `init --reset` is untouched -- it already regenerated every system-owned namespace from scratch and takes no path through the rebuild logic.
