...
- fix(ui): context-menu's submenu opens and closes on CSS alone, no JS timer (#2152). `SUB_MENU_HOVER_DELAY` (a 100ms `setTimeout`/`clearTimeout` pair, in both `context-menu.behavior.ts`'s DOM-native `bindContextSubMenu` and React's `ContextMenuSub`) is deleted outright, along with the `openTimer`/`closeTimer` refs it drove. Hovering or focusing a sub-trigger now dispatches `open`/`close` the instant the pointer or focus event fires -- exactly the shape #2148 gave tooltip/hover-card -- and the perceived hover-intent delay is a CSS `transition-delay` on the new `subContent` class (`context-menu.classes.ts`), consuming the `context-menu / subcontent / closed -> open` and `open -> closed` cells #2151 added to `motion.jsonl`: open is `duration-moderate` + `ease-enter` + `delay-hover-intent`, close is `duration-fast` + `ease-exit` with no delay. `ContextMenuClassSet` gains a `subContent` member (`context-menu-sub.astro`'s sub-content div and React's `ContextMenuSubContent` both switch onto it) separate from the parent menu's `content`, whose own open/close cells remain the unconverted #2017 component-sweep item this issue does not touch. Sub-content is never `hidden` any more (aria projection and SSR markup alike) -- a hidden node cannot transition, and the submenu now reveals purely through `:hover`/`:focus-within` over the real sub-trigger/sub-content sibling structure in the SSR-authored markup, provably with JavaScript never executing (`test/motion/reveal-candidates.test.ts` compiles the real Tailwind CSS from the real component directory and asserts the reveal selector, without calling `bindContextMenu`). Once `bindContextSubMenu`/React's `ContextMenuSub` move sub-content to `document.body` (unchanged, pre-existing behavior, needed to escape the parent's `overflow-hidden` and roving scope), that CSS sibling selector can no longer reach it, so `data-[state=open]` -- carrying the identical duration/curve/delay, with no keyboard-instant exception -- is what governs the transition from then on. `packages/design-tokens/src/generators/defaults.ts` gains matching `context-menu-subcontent-open`/`-close` entries so the pre-existing matrix-conformance test (`packages/design-tokens/test/motion-cells.test.ts`, red on main once #2151's two subcontent rows landed with no consumer) passes; the component itself does not reference the `animate-*` utilities those entries emit, matching tooltip/hover-card's own transition-based (not keyframe-based) shape. The `fade + zoom` cell's zoom/`extent-pop` half is not consumed -- same gap tooltip/hover-card already carry, since no generated utility bridges `--rafters-consumed-extent` into a `scale-*` transform without a component naming a `--rafters-*` custom property directly, which the repo's authoring rule forbids.
