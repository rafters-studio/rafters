...
- feat(registry): facet props come from the TypeScript type checker, not naming-convention regexes (#2165). `describe()` and the MCP graph now surface real props for components that previously returned `props: {}` or missed individual props: badge, container, grid, input, card, sidebar, and select all gain declared props they never published before -- badge's `variant`/`size` (an as-const array alias the union regex could not see), container's `as`/`size`/`padding`/`gap`/`columns`/`colSpan`/`rowSpan`/`position`/`depth`, grid's `preset`/`pattern`/`columns`/`role`, input's `value`/`defaultValue`/`invalid`/`errorId`, card's `as`/`fill`, sidebar's `side`/`variant`, select's `value`/`open`/`disabled`/`name`. A shared `ts.Program` (one per registry build, not one per component) resolves through alias depth (`(typeof X)[number]`), intersection types (button's `ButtonBaseProps & ...`), and body-level destructuring (grid's `const { preset } = props`). New `PropField` arms -- `boolean`, `string`, `number` -- carry the non-enum props the regex extractor could not represent (container's `query: boolean`, input's `value: string`, slider's `min`/`max`/`step: number`). A prop whose type is a union mixing literals with a structural arm now emits its literal vocabulary instead of being dropped silently (container's `columns` and `gap`, grid's `columns`, checkbox's tri-state `checked`); a union with a WIDENED primitive arm (`React.ReactNode`) still emits nothing, because its literals are examples rather than a vocabulary. Enum values are emitted in the SOURCE order of their declaration, never the checker's own union member order -- that order is keyed on global literal-type interning and reshuffles when an unrelated component joins the shared program -- with a numeric-aware sort as the fallback when a declaration's order cannot be recovered. Which props type a component resolves to is decided by NAME -- `pascalCase(<component>) + "Props"`, compared case-insensitively so input-otp's declared `InputOTPProps` matches, against the type and interface declarations in the component's own source file -- never by declaration order. Four files declare a provider or a sub-component interface above the component's own, and taking the first would publish the wrong prop surface: `<Sidebar>` would advertise `open`/`defaultOpen`/`collapsible` from `SidebarProviderProps` -- three knobs it does not accept -- beside the `side`/`variant` the two interfaces happen to share, carrying the provider's `'left'`/`'sidebar'` defaults rather than `<Sidebar>`'s own (it destructures without initializers), `<Tooltip>` would publish only the provider's `disableHoverableContent` instead of `open`/`defaultOpen`/`side`/`align`/`sideOffset`, and typography would lose `as` and `variant` to `TypographyComponentProps`. A file that declares no matching type resolves to the documented empty props object rather than to its nearest neighbour, so resizable -- which declares `ResizablePanelProps`, `ResizableHandleProps` and `ResizablePanelGroupProps` but no `ResizableProps` -- publishes `props: {}` instead of inviting an agent to set `ResizablePanel`'s group-injected internal `__resizableIndex`. Only props declared inside the component's own directory are emitted, anchored at a path segment boundary so `button-group/` is not read as part of `button/`; inherited React HTML attributes (`type`, `placeholder`, `disabled`, `className`, `onClick`) are still excluded wholesale, so input publishes its own four props rather than the hundreds `React.InputHTMLAttributes` carries. The shared program is built from the absolute component root the caller already resolved rather than from `process.cwd()`, and every discovery failure -- an unreadable root, zero component directories, zero sources -- now throws instead of yielding an empty program that would quietly hand every component `props: {}`. The react target uses the type checker; astro/vue/svelte keep the interface-body regex (`.astro`/`.vue`/`.svelte` are not valid TypeScript and cannot be fed to `ts.createProgram`), so `extractLiteralUnions`, `extractDestructuredDefaults`, `extractInterfaceProps`, and `inlineUnionValues` remain live for those targets; only `extractDestructuredNames` is removed. Defaults now resolve from the AST (boolean `true`/`false`, numeric literals) rather than a string-only regex. A new pure `propFieldToFieldDescriptor(name, field)` maps a resolved `PropField` out to kelex's `FieldDescriptor` IR so veneer and gitpress can consume rafters' published facet JSON instead of re-parsing rafters source; rafters' cross-prop `constraint` rides in the descriptor's `meta` because kelex's `FieldConstraints` is value validation with no arm for it. `rafters_describe`'s tool description now names the boolean/string/number kinds alongside enum and grammar, and `plugin/hooks/agent-contract.md` regenerates from it. The wc target's documented `props: {}` is unchanged. A destructure that renames the prop (`{ as: Element = 'div' }`, the shape container and card use) now keys its default on the prop name, so `container.as` and `card.as` carry `default: "div"` instead of no default; a forwardRef/memo callback is only read for defaults when it is bound to the component's props type. The checker sits behind an explicit `PropsTypeChecker` seam (`typeChecker.ts`), the one place the backend is chosen, and the registry-to-graph join is proven wide and recursive: every served component and composite is assembled with the real `assembleGraph` and walked through `describe()` -- 64 nodes, 214 react props, every part and edge -- against the registry item each came from.

### Bug Fixes

- fix(ui): context-menu's submenu opens and closes on CSS alone, no JS timer (#2152). `SUB_MENU_HOVER_DELAY` (a 100ms `setTimeout`/`clearTimeout` pair, in both `context-menu.behavior.ts`'s DOM-native `bindContextSubMenu` and React's `ContextMenuSub`) is deleted outright, along with the `openTimer`/`closeTimer` refs it drove. Hovering or focusing a sub-trigger now dispatches `open`/`close` the instant the pointer or focus event fires -- exactly the shape #2148 gave tooltip/hover-card -- and the perceived hover-intent delay is a CSS `transition-delay` on the new `subContent` class (`context-menu.classes.ts`), consuming the `context-menu / subcontent / closed -> open` and `open -> closed` cells #2151 added to `motion.jsonl`: open is `duration-moderate` + `ease-enter` + `delay-hover-intent` + `extent-pop`, close is `duration-fast` + `ease-exit` with no delay. Both directions are true to the cell's "fade + zoom" movement -- `extent-pop` picks the scale member and the new `scale-(--rafters-consumed-extent)` parens-shorthand (Tailwind v4's CSS-variable form of an arbitrary `scale` value) reads the alias back, so no component-owned `scale-95` literal and no `var(--rafters-...)` call appears in source, satisfying the "consumers never reference `--rafters-*` vars directly" rule the same way `duration-*`/`ease-*`/`delay-*` already do. `ContextMenuClassSet` gains a `subContent` member (`context-menu-sub.astro`'s sub-content div and React's `ContextMenuSubContent` both switch onto it) separate from the parent menu's `content`, whose own open/close cells remain the unconverted #2017 component-sweep item this issue does not touch. Sub-content is never `hidden` any more (aria projection and SSR markup alike) -- a hidden node cannot transition, and the submenu now reveals purely through `:hover`/`:focus-within` over the real sub-trigger/sub-content sibling structure in the SSR-authored markup, with no `setTimeout` deciding when (`test/motion/reveal-candidates.test.ts` compiles the real Tailwind CSS from the real component directory and asserts the reveal selector and the extent-pop rules, without calling `bindContextMenu`; this is not a no-JS-floor claim -- spec correction 2026-08-28 -- since the parent menu itself opens only on the `contextmenu` event, which requires script). Once `bindContextSubMenu`/React's `ContextMenuSub` move sub-content to `document.body` (unchanged, pre-existing behavior, needed to escape the parent's `overflow-hidden` and roving scope), that CSS sibling selector can no longer reach it, so `data-[state=open]` is what governs the transition from then on -- but not with one blanket delay (fixed post-review, #2187): the `context-sub-menu` score now threads an input-source mark through the `open` action's payload (`'pointer'` from a genuine `pointerenter`, `'discrete'` from a click or the ArrowRight/Enter/Space keyboard open), the aria projection puts it on `subContent` as `data-open-source`, and `context-menu.classes.ts` scopes `delay-hover-intent` to `data-[state=open]:data-[open-source=pointer]:` so a click or keyboard open resolves through the un-delayed `duration-moderate`/`ease-enter` pair instead -- keyboard navigation stays exactly as fast as it was before this issue. A `pointerenter` on the panel itself still re-asserts `open` with the same `'pointer'` mark (mirrored between the DOM-native bind and React, matching the "one score, three performances" contract) so a pointer that outruns the now-undelayed `pointerleave` recovers rather than latching closed. `subContent` also carries `aria-hidden="true"` while closed (SSR markup and the aria projection alike, #2187) -- dropping `hidden` above meant a closed panel was still a live `role="menu"` node with `role="menuitem"` children in the accessibility tree, reachable by a screen reader's browse-mode cursor even though `tabindex="-1"` already kept them out of the tab order; `aria-hidden` has no layout or interaction effect, so neither the CSS reveal nor the transition changes. `startContextSubMenuEffects`'s teardown (shared by the DOM-native bind and React) now also resets every item back to `tabindex="-1"` on close, since roving-focus's own cleanup only detaches its listeners and otherwise would have left one item at `tabindex="0"` -- a focusable descendant of the now aria-hidden panel, which is exactly what axe's aria-hidden-focus rule catches (`context-menu.element.conformance.test.ts` asserts axe stays clean after an open/close cycle, not just at mount). Per the issue's spec correction (2026-08-28), `packages/design-tokens` gains no new entries: the two subcontent cells #2151 added to `motion.jsonl` are consumed here entirely as generic leaf tokens (`duration-fast`/`ease-exit`/`duration-moderate`/`ease-enter`/`delay-hover-intent`/`extent-pop`), not through a `DEFAULT_MOTION_CELL_ANIMATIONS` entry or an `animate-*` keyframe utility. For WC/Astro, subcontent stays permanently mounted and transitions between states, so no keyframe shape applies there; React's `ContextMenuSubContent` unmounts via `usePresence` once the close transition ends, which is a genuine presence change for that performance, but the spec correction still rules no `packages/design-tokens` addition regardless. The pre-existing matrix-conformance test (`packages/design-tokens/test/motion-cells.test.ts`) gains the two rows as a named exclusion instead, since main was already red on them the moment #2151's rows landed with no consumer and no exclusion.
...
- fix(color-utils): the gamut numerics PR #2141 changed are now disclosed and pinned by regression tests (#2159). #2141 replaced the deleted `packages/ui/src/primitives/oklch-gamut.ts` hand-rolled sRGB/P3 matrices with `@rafters/color-utils`' `colorjs.io` wrap and called the two "numerically equal" without a test that could tell them apart. THEY ARE NOT EQUAL, and anyone who pinned a boundary value against the old math should re-read it: over a 45,360-sample OKLCH grid the two implementations disagree on sRGB membership for 3.2% of samples and on the P3/out split for 5.3%, and `findMaxChroma` at `h=145, l=0.05` was 8.5x too permissive before the fix (0.1937 against the corrected 0.0227). Two independent causes, both fixed by the wrap: the old `EPS = 0.001` tolerance was applied to LINEAR-RGB channels, where near black it is larger than the gamut itself, so plainly undisplayable colors came back in-gamut (`computeGamutBoundaries(160)` at `l=0.04` returned above 0.26 for both curves where the real answers are `maxC_srgb` 0.0094 and `maxC_p3` 0.0133); and the old P3 blue row (`-0.026073181, -0.703486028, +1.729559209`) was the wrong matrix's shape, which sat the P3 boundary well inside the real one (`h=145, l=0.85` gives `maxC_p3` 0.3625, not the 0.3047 the old code reported). `computeGamutBoundaries` and `getGamutTier` gain regression cases at exactly the points where the two implementations disagree -- the pre-existing `{ l: 0.7, c: 0.25, h: 150 }` p3-tier pin passes against both and proved nothing -- and a new structural test fails the build if any file under `packages/ui/src` or `packages/color-utils/src` ever declares a gamut matrix coefficient again. `findMaxChroma` deliberately stays a bisection over `inGamut`: colorjs.io 0.5.2's `toGamut({ method: 'oklch.c' })` returns a CLIPPED color, whose chroma read back at the requested lightness overshoots the boundary (0.0284 against 0.0094 at `h=160, l=0.04`), so it cannot answer this question. No public API changed.
- fix(ui): `packages/ui/src/old/` is excluded from the ui tsconfig and from vitest discovery (#2159). That tree is the quarantined pre-rewrite surface, outside everything the registry serves and under a standing no-edits rule, but `tsconfig.json`'s blanket `src/**/*` include compiled it and `vitest.config.ts`'s `src/**/*.test.{ts,tsx}` / `src/**/*.a11y.{ts,tsx}` globs ran its tests -- which is exactly why quarantined files kept turning up as ordinary edit targets. `src/old/**/*` joins the tsconfig `exclude`, `src/old/**` joins vitest's test and coverage excludes, and the 38 `.test` plus 13 `.a11y` files under it leave the ui suite -- 51 files carrying 838 passing and all 6 skipped tests at time of exclusion, measured before and after, so the drop is a recorded number rather than a silent one. Forward-looking only: nothing under `src/old/` is edited or reverted.

## 0.3.0

### Breaking changes

- **Tooltip, HoverCard, and NavigationMenu no longer take any timing prop, and none of them uses a JavaScript timer (#2148).** Motion in rafters is CSS and tokens only: hover-intent on open and linger on close are `transition-delay` over the emitted `delay-hover-intent` / `delay-linger` utilities, the reveal is native `:hover` / `:focus-visible` / `:focus-within` over each component's real sibling structure, and all three now work with JavaScript turned off. The primitives that used to implement this in TypeScript -- `lib/primitives/hover-delay.ts` and `lib/primitives/motion-tokens.ts` -- are deleted outright, with no replacement, and `rafters add tooltip` / `add hover-card` / `add navigation-menu` no longer resolve either as a dependency. What a consumer has to change:
  - **Removed props.** `Tooltip` loses `delayDuration` and `skipDelayDuration` (on `Tooltip`, on `Tooltip.Provider`, and as the Astro `delayDuration` / `skipDelayDuration` props and their `data-delay-duration` / `data-skip-delay-duration` attributes). `NavigationMenu` loses `delayDuration` (React prop, Astro prop, and `data-delay-duration`). `HoverCard` loses `openDelay` and `closeDelay` (React props, Astro props, and `data-open-delay` / `data-close-delay`). Delete the prop; the system token applies. There is no per-instance override, by design -- a hover-intent delay is a system decision, not a tuning knob.
  - **The React `Tooltip` and `HoverCard` are un-portaled by default.** `TooltipRoot` and `HoverCardRoot` now render a real wrapper element (matching their Astro roots) with trigger and content as DOM siblings, and `Tooltip.Content` / `HoverCard.Content` render unconditionally instead of returning `null` while closed and portaling to `document.body`. A CSS reveal cannot reach a node that does not exist or that lives outside its trigger's parent. `<Tooltip.Portal>` / `<HoverCard.Portal>` remain as the explicit escape hatch for a clipping ancestor -- reaching for one opts that instance out of the CSS contract, and it is then revealed by `data-state` alone. `Content` no longer takes `forceMount` or `container` (both existed only for the automatic portal); pass `container` to the explicit `Portal` instead. Content is also never given the `hidden` attribute any more, in any performance: visibility is opacity + pointer-events.
  - **`pointer-events` is transitioned, not switched, and NavigationMenu's dismissal flag moved.** The reveal rule no longer owns `pointer-events`: the base rule transitions it (`transition-[opacity,pointer-events]` + `transition-discrete`) so it flips at the fade's halfway point instead of the instant `:hover` drops. Without that, hoverable content -- the default -- latched closed the moment the pointer crossed the 4px `sideOffset` gap between trigger and content, and HoverCard's panel was fully opaque and fully click-through for its entire 300ms linger. A browser without `transition-behavior` falls back to the immediate flip. Separately, NavigationMenu's WCAG dismissal flag is now `data-dismissed="true"` on the dismissed PANEL (`[data-part="content"]`) rather than on the `<nav>` root: root-scoped, one Escape blanked every panel in the bar and made every trigger inert until the pointer left the whole menu. Consumers who wrote their own CSS or tests against `nav[data-dismissed]` must read the panel's attribute instead; `setNavigationMenuDismissed` now takes the panel element, and `clearSettledNavigationMenuDismissals(root)` / `navigationMenuPanel(root, value)` are exported alongside it. A dismissal is dropped only once nothing can still reveal that panel -- its item is neither `:hover` nor holding focus -- because a dismissed panel whose trigger still has the focus Escape returned to it would otherwise come back at opacity 1 against `aria-expanded="false"` the moment the pointer left the bar (WCAG 1.4.13). Tooltip and HoverCard clear their root-level flag on the same condition.
  - **NavigationMenu's own chrome is retuned onto the generated motion utilities.** The trigger and link hover transition was a hand-written `transition-colors duration-150 motion-reduce:transition-none`; it is now `motion-hover`, the emitted utility for the interactive-surface hover cell (color, fast, standard). The chevron and the indicator were `transition-transform duration-200`; both are now `motion-toggle` (transform, moderate, standard), which moves them from 200ms to the moderate token's 250ms. No literal millisecond value and no component-level reduced-motion query survives in these files -- reduced motion is the token sheet's own responsibility.
  - **A forced-open reveal is immediate.** The `data-[state=open]` path -- a controlled `open`/`value`, a keyboard open, a tap on a device with no hover -- carries the open cell's duration and curve but no `delay-hover-intent`. Hover-intent filters accidental pointer transit; there is no accidental Enter press. The hover path is unchanged: its reveal rule is more specific, so hovering still waits out the intent delay.
  - **Value changes.** Tooltip's close no longer waits: it read `--rafters-delay-linger` (300ms) for its close timer, which was drift against the motion matrix, and now closes immediately over the fast duration and exit curve. NavigationMenu's close no longer waits either: its hover-intent timer reused the OPEN delay (200ms) on close, also drift, and closing is now immediate on the same fast/exit cell. HoverCard is the one component whose close still carries a delay (`linger`, 300ms -- numerically unchanged from the old `DEFAULT_CLOSE_DELAY`, but now sourced from the token). HoverCard's OPEN delay moves from its 700ms default to the `hover-intent` token's 200ms -- a deliberate value change, not just a mechanism change.

### Features

- feat(design-tokens): motion cells gain a loop form, and cell coverage grows from 6 animated cells to 47 (#2154). A `motion-cell-*` token's duration is now the same tagged union `motion.jsonl` states -- `{"kind":"tier","tier":"normal"}` or `{"kind":"period","period":"shimmer"}` -- replacing a bare `durationTier` string that had no way to say "this cell loops". The four looping rows of the matrix therefore have a representation and a compiled rule for the first time: `animate-skeleton-root-waiting`, `animate-spinner-root-busy`, `animate-progress-root-indeterminate` and `animate-input-otp-caret-idle` emit `animation-duration: var(--rafters-period-<member>)` with `animation-iteration-count: infinite`, and no `animation-timing-function`, because every period row in the matrix declares `curve: {"kind":"none"}` and naming one would be an assignment no cell made. A tier-kind cell writes no iteration count at all -- the CSS initial value is already 1, and the absence is what says a transition runs once. THE REDUCED-MOTION LAW NOW REACHES CELLS AS SET MEMBERSHIP: a period-kind cell carries `reducedMotionAware: false` and gets no `@media (prefers-reduced-motion: reduce)` block, so loops slow but never stop, while every tier-kind cell keeps mechanism B (zeroed `animation-duration`, which preserves the keyframe's end state and still fires `animationend` for the presence contract). The field, not the JSON spec, is what the exporter reads, so an operator pin (`rafters set motion-cell-spinner-root-busy "<shorthand>"`) keeps its exemption instead of silently falling under the zeroing law. 41 new `animate-<component>-<part>-<transition>` utilities land across dialog, sheet, drawer, sidebar, alert-dialog, context-menu, select, combobox, command, navigation-menu, tooltip, hover-card, calendar, tabs, avatar, image, embed, alert, scroll-area, skeleton, spinner, progress and input-otp, each transcribed from its matrix row and asserted against `motion.jsonl` member for member by a new conformance test. NO NEW KEYFRAME IS AUTHORED: every entry names a shape that already exists, so no geometry enters the system in a coverage change -- the rows whose movement has no existing shape (drawer content's per-side slide, carousel travel, the discrete swaps) are left uncovered rather than approximated, because an approximated shape is a value nobody chose. WHICH ROWS ARE LEFT UNCOVERED IS NO LONGER PROSE: all 95 of them are enumerated one line each, under one of six reasons, in `EXCLUDED_ROWS` (`packages/design-tokens/test/motion-cells.test.ts`), and the suite asserts that cells-assigned minus cells-consumed equals that list EXACTLY -- so a new matrix row fails CI until somebody either transcribes a cell for it or writes down why not, and an exclusion that outlived its row fails the same way. A cell whose `duration.kind` is neither form now fails the generator by name and coordinates, and a period member that is not a declared leaf fails the exporter, rather than either compiling to a `var()` that resolves to nothing.
- feat(plugin): the `rafters-frontend` skill now leads with why the assembly-model rules hold instead of a bare list of prohibitions (#2172). The opening reframes from "the system makes design choices, you assemble" to the reflex correction "this is not Tailwind, and it is not shadcn," followed by the distinction the whole system rests on: the designer's judgment is encoded in token values and applied in components and composites, so reaching for a pattern carries the judgment with it while reaching past it to a raw value (`bg-primary`, `text-4xl`, `gap-4`) strips the judgment off. The "Never" table collapses from twelve separate prohibitions into one rule with twelve faces, most rows naming what they drop (`text-4xl` drops the type scale, `gap-4` drops the spatial rhythm). The `src/components/` authoring exemption -- the one legitimate place to touch raw tokens -- is explained rather than left as an unstated carve-out, and `attentionBudget` is promoted from a config bullet to the agent's primary check: whether a screen asks the reader to hold too much, whether reading order and priority order agree. `allowed-tools` is removed from the frontmatter, since the skill's central instruction is to query the MCP rather than use file/shell tools directly.
- feat(cli): `@rafters/shared`, `@rafters/color-utils`, and `@rafters/math-utils` now ship in lockstep with the `rafters` CLI, and `rafters add` pins them to that exact version (#2144). `sync-version.ts` gains three new JSON targets (the three packages' `package.json`) and a new target kind -- a `CONSTANT_TARGETS` loop that rewrites `export const RAFTERS_VERSION = '...'` in `packages/shared/src/version.ts` by regex, throwing `"<path>: expected RAFTERS_VERSION exactly once, found <n>"` if the declaration is missing or duplicated. Both package manifests gain `homepage`, `license`, `repository`, `sideEffects: false`, and `LICENSE` in `files` (copied verbatim from `packages/cli/LICENSE`), and carry no `private` flag, so `pnpm publish` can ship them. `installRegistryDependencies` now forwards every dependency whose name starts with `@rafters/` as `<name>@<VERSION>` (the CLI's own runtime version from `version.ts`), discarding whatever version the registry file declared -- a consumer's installed code always matches what the CLI was built against, closing the version-drift gap `@rafters/color-utils@0.1.0` left open when that literal predated the package actually reaching `0.2.3`. `release.yml`'s publish order is now math-utils, shared, color-utils, then the CLI, so a dependency publish failure stops the job before the CLI (whose `add` pins to a version that would 404) reaches npm; the build step (which runs `sync-version.ts`) stays first. `RAFTERS_VERSION`'s doc comment is corrected: it is the lockstep suite version this script writes, not a value the CLI's `package.json` reads.
...
- fix(ui): the color-picker's OKLCH gamut math is single-sourced onto `@rafters/color-utils`, closing a real fusion bug (#2133). The deleted `packages/ui/src/primitives/oklch-gamut.ts` hand-rolled its own OKLCH-to-linear-RGB gamut checks behind a private `EPS = 0.001` tolerance, and its `findMaxChroma` fused the sRGB and P3 boundaries into one curve (`inSrgb(l,c,h) || inP3(l,c,h)`), so every caller asking for a maximum-chroma boundary was already silently receiving the P3 answer regardless of which gamut it meant to probe. `color-area.ts` and `hue-bar.ts`'s vivid sweep now call color-utils' `computeGamutBoundaries` (backed by `colorjs.io`'s `inGamut`, a different matrix and a tighter tolerance than the deleted inline math) and read `maxC_p3` explicitly, which reproduces the same P3-boundary curve the fusion bug already rendered there. `color-picker-state.ts` drops its own `getGamutTier` for color-utils' export, and `hue-bar.ts`'s plain membership test moves onto color-utils' `isInSRGBGamut`/`isInP3Gamut`; both were already sequential (sRGB checked before P3), never fused, but they now inherit colorjs.io's gamut boundary rather than the hand-rolled one, so a color sitting near the sRGB/P3 edge can tier differently by the sub-tolerance amount the two implementations disagree on -- a real, if small, numeric shift in where the P3 boundary falls, not the "no observable behavior change" the fix originally claimed for itself. A new regression test pins a color inside P3 but outside sRGB (`{ l: 0.7, c: 0.25, h: 150 }`) to the `p3` tier. `hueFromBarPos`/`barPosFromHue` -- pure UI layout math for the hue-bar widget, with no color-utils analog -- move verbatim to a new local `hue-warp.ts`. `packages/ui` gains a `workspace:*` dependency on `@rafters/color-utils`.
- fix(mcp): `presence` (`installed` | `available`) is now measured from disk instead of read from `config.installed` (#2171). That field is a record `rafters add` maintains, and a record drifts from the thing it records in both directions: a part pulled in only as a dependency never appeared in it (`card-action` is on disk in every project that installed `card`, and reported `available`), and an entry survived a manual delete. Presence is the join between the public catalog and the project, so it is measured now -- a `readdir` of the resolved `componentsPath` / `primitivesPath` / `compositesPath`, with an id taken as the basename before the first dot so every file a component ships (`input.tsx`, `input.behavior.ts`, `input.classes.ts`) maps to one id and any of them proves it is there. The fallback folders used when a workspace has no config at all now come from the same `KIND_PATHS` map `--update-all`'s disk reconciliation and `rafters init` already use, rather than a second, drifted copy of those literals that pointed at a path (`.rafters/composites`) no framework's default actually writes. The scan is deliberately NOT cached while the catalog behind it is: the catalog is a network fetch plus a graph build, the scan is a directory listing, and caching the cheap half is exactly what would have made `rafters add` invisible until an MCP restart. A malformed path field in `config.rafters.json` (a `componentsPath` that is a number, say) now returns a structured error naming the field instead of throwing. The MCP stays read-only. Reported by shingle.
- fix(mcp): `rafters_generate` now reports whether the component it returns is actually installed (#2171). The response gains `presence` (`installed` | `available`) on EVERY call, plus an `install` command (`rafters add <name>`) when it is `available`. Previously generate returned `{ component, target, snippet, slots }` and omitted the one field that decides whether the snippet is usable: it would hand back `<Dialog>...</Dialog>` for a workspace with no `dialog` installed, the agent would paste it, and the build would die on a missing import with nothing in the payload to have warned it. `presence` is sourced from the same disk-scanned installed set `rafters_describe`'s overlay stamps describe with, so the two tools cannot disagree about one workspace by construction. generate still always returns the snippet -- it is the authority on the artifact, not a gatekeeper, and withholding correct code over a missing install step would make it useless to the agent it exists to serve. Reported by shingle's 0.2.3 eval.
- fix(mcp): the `rafters_describe` and `rafters_generate` tool descriptions no longer claim things the code does not do (#2171). The natural-language address door is now documented as it actually ships -- wired, deterministic keyword matching over a small curated tag axis, refusing below its match threshold with a note pointing the caller at `describe(components)` rather than guessing -- correcting an earlier pass that overcorrected a stale "descoped for v1" claim by deleting the sentence outright instead of fixing it. The stale `.vocab` address form is dropped from `describe`'s description; drilling into a prop already returns its values inline. `generate`'s description no longer tells an agent not to wrap its returned snippet with children -- every react facet carries `slots: []` unconditionally, and generate still snippets content-wrapped usage (`<Button>Save</Button>`), so the old sentence was exactly backwards for its primary target. `describe`'s description now says a part node carries `parent`, plus `siblings` only when its parent has other parts (a root node, its own example two clauses earlier, carries neither), and names all three child pointer types (`prop`, `part`, and `edge` for `composesWith`) instead of two. `generate`'s refusal now carries the same near-miss note the intent door produces, matching what its description already promised. And `describe(button.props.fill)` no longer claims to return "the real token values" -- `fill` is a grammar prop, and a grammar prop's `vocab` is deliberately stripped before it reaches the agent surface; drilling it returns composition rules, never inlined values. The description now drills a real, tested prop (`describe(button.props.variant)`) and moves its probe example to `describe(button.props.color.?)`, the exact miss case pinned to `null` in `graph.test.ts`. `plugin/hooks/agent-contract.md` and the plugin bundle are regenerated in the same commits, since both are built from these descriptions.
- fix(plugin): the agent contract no longer instructs agents to do something the pre-edit hook denies (#2171). It said "use semantic color tokens (bg-primary, text-accent)" while `pre-edit.sh` rejects exactly those utilities in assembly code; the contract now points at the composite block's variant meta or the component's `variant` prop, which is where the color decision actually lives. The hook's own denial message was also naming three deprecated MCP tools (`rafters_pattern` / `rafters_composite` / `rafters_component`) and now names `rafters_describe`.
- fix(ui): `.classes.ts` files no longer choose a timing value in TypeScript -- every stock Tailwind duration literal (`duration-150`, `duration-200`, `duration-100`, `duration-300`) across ~27 component class sites is replaced with the matrix-assigned tier utility read off its cell in `packages/ui/docs/spec/matrix/motion.jsonl` (#2153). Most hover/colour sites map `150`/`100` -> `duration-fast` (breadcrumb, button, collapsible trigger, dialog/drawer/sheet/popover close, item, nav-menu trigger, pagination, scroll-area scrollbar, sidebar x4, slider thumb, tooltip). `tabs` is a real, visible timing correction: its indicator/panel active-change cells (`motion.jsonl:6-7`) assign tier `fast`, not the `duration-200` (`moderate`-equivalent) it shipped with. Three sites land SLOWER than their old literal because their governing cell is the checked/pressed-state colour swap, not the undocumented hover they also carry: `checkbox`, `radio-group`, and `toggle` move `150` -> `duration-moderate`. Five sites land FASTER: `input`, `input-group`, `textarea`, and `input-otp` move their focus-ring `100`/`150` -> `duration-micro`, each per its own `root/focus` cell. `select`'s trigger joins them at `duration-micro` too, but by analogy rather than a direct cell: its own matrix cell (`select | trigger | hover`, `fast`) governs a border-colour change the class does not actually transition, while the `transition-shadow` it carries only animates the focus-visible ring -- the same movement `input`/`input-group`/`textarea` source from their `root/focus` cells, all of which are `micro`. No `select | trigger | focus` cell exists yet to source this directly; adding one is matrix hygiene (#2158/#2159), not this fix. `switch`, `select` chevron, `toggle-group`, `collapsible` content, and `progress` fill move to `duration-moderate`. Four sites have no matching matrix cell and are deliberately left on their stock literal, deferred to the matrix-hygiene issue (#2158): `badge` (no `badge` component in the matrix at all) and three `navigation-menu` sites (`link`, `chevron`, `indicator` -- only `trigger/hover` and the `panel` cells exist for that component).

### Refactors

- refactor(ui): `usePresence` observes animations instead of measuring and timing them (#2157). The hook shipped by `rafters add` no longer reads `animation-duration` / `animation-delay` / `transition-*` out of `getComputedStyle`, parses those CSS time strings in TypeScript, or runs a derived `setTimeout` against the result. It calls `node.getAnimations()` after the `data-state="closed"` commit and awaits `Promise.allSettled(animations.map((a) => a.finished))`. `timeMs`, `longestRun`, `ExitMeasurement`, `measureExit`, and `fallbackMs` are deleted outright, and with them the second clock that ran alongside the browser's: the exit's end is now the browser's own answer rather than a margin (`x1.5 + 50ms`) applied to a number re-derived from CSS. The public interface -- `present`, `ref`, `state` -- is unchanged, `disclosable` is still the only writer of `data-state`, and no consuming component changed. Three behavioral consequences a consumer can observe. (1) The interrupted-enter race no longer needs an animation-name filter: closing mid-enter cancels the enter, a cancelled animation is not returned by `getAnimations()`, and a rejected `finished` is absorbed by `allSettled` rather than collapsing the wait -- verified in chromium, firefox, and webkit by `test/presence/presence-race.e2e.ts`, which drives the real hook through esbuild plus `page.setContent` and asserts the release lands inside the exit's window and nowhere near the interrupted enter's. (2) A node with nothing attached still releases synchronously, unchanged. (3) BEHAVIOR CHANGE under `prefers-reduced-motion`: mechanism B (#2017) zeroes `animation-duration` while leaving the animation attached, and a zero-duration animation finishes inside the style flush that creates it, so `getAnimations()` reports nothing and presence releases in the same tick instead of on the next frame's `animationend`. That is the same instant close, one frame earlier, and it is measured at the platform level (no React involved) in the same spec. There is NO backstop timer any more and none was needed: the only shape that would never settle is an infinite loop animation on a presence-gated node, and every `period-*` cell in the motion matrix belongs to input-otp's caret, progress, skeleton, or spinner -- none of which is presence-gated. If one ever lands on an overlay content part, a backstop would be an engineering failsafe outside the value system, never derived from a duration tier.
- refactor(design-tokens, ui): every hand-rolled OKLCH-to-CSS-string formatter in `packages/design-tokens` and `packages/ui` now delegates to `oklchToCSS` from `@rafters/color-utils` (#2147), so `rafters add`'s delivered source for `color-swatch`, `color-scale`, `cvd-simulation`, `color-area`, and `hue-bar` changes: their `oklch()` string formatting now goes through `oklchToCSS` instead of a local template literal. `color-swatch.ts`'s alpha-aware `toOklch` is byte-identical (alpha now flows through `oklchToCSS` automatically). The four plain-formatting `packages/ui` sites (`color-scale`, `cvd-simulation`, `color-area`, `hue-bar` x2) are byte-identical by construction -- none of them rounded before, `oklchToCSS` with no `precision` option doesn't either, and each call site explicitly passes `alpha: 1` (their shared `OklchColor` type has no alpha field to begin with, but the override is kept as an explicit guarantee rather than an implicit one). `design-tokens`'s `generators/color.ts` site is unchanged in shape: it was already alpha-aware before this refactor and stays that way. `design-tokens`'s `tailwind.ts` site drops its local `formatNumber` (round to 3 decimals, then strip trailing zeros) for a call that pre-rounds each channel with `toFixed(3)` and forces `alpha: 1` before handing off to `oklchToCSS` -- this reproduces formatNumber's exact round-then-strip behavior AND its unconditional alpha drop for every OKLCH the exporter can receive, not only the already-rounded, alpha-less output `generateOKLCHScale`/the hardcoded defaults produce; a fixture test importing a full-precision, alpha-carrying seed (`rgba(59,130,246,0.5)`) pins this. All four `design-tokens` sites now force `alpha: 1` before calling `oklchToCSS`, matching the old formatters, none of which ever emitted a fourth channel -- `dtcg.ts` (no precision option) and `tailwind.ts` are therefore fully byte-identical. `typescript.ts` diverges on exactly one remaining axis: it previously formatted L/C to 3 decimals and H to 1 decimal separately, and `oklchToCSS`'s single `precision` option cannot reproduce mixed per-channel precision, so H now serializes at 3 decimals instead of 1 (`{precision: 3}` applies to every channel) -- a non-round hue that used to round away to one decimal (e.g. `187.3`) now surfaces its real value (`187.348`) rather than gaining padded zeros; the color value itself is unchanged. This is tracked as a follow-up on issue #2147 rather than fixed here, since no single `oklchToCSS` call can express mixed per-channel precision. Each divergence (or its absence) is pinned by a fixture test over an `importColorFamily`-built alpha-carrying seed, one per exporter (`tailwind.test.ts`, `dtcg.test.ts`, `typescript.test.ts`). Each of the five changed `packages/ui` primitives now carries an `@internal-dependencies @rafters/color-utils` JSDoc tag.
- refactor(color-utils): one temperature classifier, an `oklchToCSS` that can express alpha and precision, and about 520 lines of dead code gone (#2146). Every `ColorValue` carried two contradictory warm/cool/neutral verdicts: `analysis.temperature` came from `getColorTemperature` (warm 0-90 and 315-360, cool 150-270, neutral below chroma 0.04) while `atmosphericWeight.temperature` came from a separate inline hue-range check inside `calculateAtmosphericWeight` (warm 0-60 and 300-360, cool 180-270, no chroma guard). They disagreed on 148 of the 360 integer hues, including the color wheel's own `warning` (h=85) and `success` (h=145) anchors. `calculateAtmosphericWeight` now sources its verdict from `getColorTemperature`, which is the package's only temperature classifier. BEHAVIOR CHANGE: on those 148 hues `atmosphericWeight.temperature` reports a different value than before, and because the same warm/cool verdict also feeds the `+0.3` / `-0.2` term in `distanceWeight`, `atmosphericWeight.distanceWeight` and `atmosphericWeight.atmosphericRole` shift on those hues too; low-chroma colors (c < 0.04) now read `neutral` where they previously read warm or cool by hue alone. `analysis.temperature` is unchanged. `oklchToCSS(oklch, options?)` gains an optional second argument: it now emits `oklch(L C H / A)` when `alpha` is defined and is not 1, and `options.precision` sets the decimal places on every emitted channel. Called with no options on an alpha-less color it is byte-identical to before -- pinned by test, because the design-tokens generators emit that exact string. The visible consequence: a design-tokens import carrying a non-1 alpha (an `rgba()` or `#RRGGBBAA` source) now emits the fourth channel where it previously dropped it silently. Removed from the public surface with zero callers repo-wide: `validateSemanticMappings` and its `AccessibilityAlert`/`SemanticMapping` types (the whole `validation-alerts` module, which also carried a second, incompatible delta-E formula and a second semantic-hue table that disagreed with `semantic.ts`), `findAccessibleColor` (whose bisection loop never applied the bisected lightness before re-testing, so it was broken as well as dead), `lighten`, `darken`, `adjustChroma`, `adjustLightness`, `blendColors`, `generateSurfaceColor`, `generateNeutralColor`, `calculateColorDistance`, `oklchToHex`, `meetsWCAGStandard`, and `meetsAPCAStandard`. Every name the CLI, design-tokens, studio, ui, and api consume keeps its signature and stays exported.
- refactor(math-utils): `@rafters/math-utils` is pruned to exactly the surface `@rafters/design-tokens` consumes -- `resolveRatio`, `ratioValue`, `tryParseUnit`, `generateModularScale`, `evaluateExpression`, and the `Ratio`/`Unit` schemas and default registries (#2145). `progression`, `generateSequence`, `linearSequence`, `exponentialSequence`, `generateFibonacciLike`, `calculateProgressionStep`, `findClosestProgressionStep`, `interpolate`, `formatUnit`, `calculateWithUnits`, `convertUnit`, and the unused `expressions.ts` module are removed as dead surface; `findRatio` and `findUnit` become unexported internals. The package now publishes to npm in lockstep with the CLI (version, license, repository, and `sideEffects` fields added; `LICENSE` copied in), joining `@rafters/shared` and `@rafters/color-utils` (#2144) on every `v*` tag release.
...
- feat(plugin): installing the rafters Claude Code plugin now auto-wires the `rafters` MCP server at a version pinned to the plugin itself, shipped as a single bundled artifact -- no `pnpx`, no `@latest`, no dlx cache in the launch path, and no manual MCP registration (#2124). `plugin.json` declares `mcpServers.rafters` (command and args separate) pointing at a self-locating `plugin/bin/rafters` bash wrapper, which runs one committed, self-contained ESM bundle `plugin/bin/rafters-mcp.bundle.mjs` (~1.3 MB, no chunks). The bundle is built by a third tsup target from a new MCP-only entry `src/plugin-entry.ts` that never imports the studio/add/init heavy paths. This is the same stale-dlx-cache class of failure `rafters --version` fixed at #2021, closed at the source: the launch path no longer resolves anything from the network. Bundling into one file forced a real structural fix -- `RaftersConfig`/`FontsConfig`/`migrateConfig` moved out of `commands/init.ts` into a leaf module `src/config/rafters-config.ts` with no `@rafters/design-tokens` edge, because that package's transitive `css-tree` loads JSON data via a module-scope `createRequire` that cannot be inlined into a single-file build; `mcp/tools.ts` and `add.ts` import the leaf, and `init.ts` re-exports the types and `migrateConfig` so `reconcile.ts` and the tests keep resolving. A `plugin/hooks/version-check.sh` appended to the existing SessionStart startup group warns when the on-disk bundle desyncs from `plugin.json`'s declared version, and version lockstep is enforced across `plugin.json`, `marketplace.json`, and `plugin/package.json`. The standalone `rafters mcp` / `rafters --version` from a plain npm install are unaffected -- the `src/index.ts` entry and `version.ts` are untouched. `packages/cli/test/plugin-bundle.test.ts` guards the invariants: one file with no chunks, no `css-tree`/`mdn-data`/`@tailwindcss/cli` strings, and a real `initialize` + `tools/call(rafters_describe)` against a fixture workspace returns a target-resolved node with empty stderr.
- feat(cli): `rafters agents` writes an `AGENTS.md` contract for non-Claude-Code agent hosts, and the Claude Code plugin's session-start hook now injects the same contract from a committed, generated file (#2125). One generator (`generateAgentContract`) builds the assembly-model rules plus a tool roster from the LIVE `TOOL_DEFINITIONS`, filtering on the `[DEPRECATED` description prefix so a tool rename or removal changes the contract with no hand edit; the surrounding prose never names a specific tool, so it cannot drift from the roster. `rafters agents` requires a `.rafters/config.rafters.json` at cwd (throws pointing at `rafters init` otherwise) and merges the contract into `AGENTS.md` between `<!-- rafters:agent-contract:start -->` / `<!-- rafters:agent-contract:end -->` markers -- replacing only the marked span on re-run, appending a fresh block when markers are absent or corrupted, never attempting a partial repair. The generated `plugin/hooks/agent-contract.md` is committed and kept byte-identical to the generator by a lockstep test and the CLI `build` step. This also removes a real bug: `session-start.sh` previously hardcoded a call to `rafters_vocabulary`, a tool that does not exist in `TOOL_DEFINITIONS`; the contract now names only live tools. `AGENTS.md` instructs the lockfile-pinned local binary (`pnpm exec rafters mcp` / `npx rafters mcp` / `yarn rafters mcp`), not a network-fetched `@latest`/dlx copy. `init`/`add`/`mcp`/`studio` are unchanged.
- feat(editor): `createEditorHistory` -- the block editor's undo/redo core, composing #2105's `applyOp`/`applyOpSequence` over a single `createMemory` cell holding `{ doc, sel, done, undone }` (#2104). It is an own client (`memory`, `canUndo`, `canRedo`, `controls`), not a Slice and with no `dispatch()`, per the pinned interface contract. `undo`/`redo` replay a `HistoryEntry`'s flattened `EditorOp[]` sequences (`.inverses` / `.ops`) through `applyOpSequence`, so a group unwinds and rewinds as one; `canUndo`/`canRedo` are get-accessors computed from `done`/`undone` length, never stored fields that could drift. Coalescing merges consecutive same-kind text edits into one entry until a `closeGroup` boundary, a cap evicts the oldest entries once history exceeds its limit, and `applyExcluded` mutates the doc while bypassing history entirely. `insertText`/`removeText` canonicalize markless content back to a plain string (`collapseIfPlain`), and `controls.apply()` synthesizes a `removeText` for a live non-collapsed selection before an `insertText` targeting it, so replacing selected text is one coherent history entry. A p95 undo/redo benchmark on a 1000-block document guards the perf floor.
- feat(editor): `bindEditor` -- the controlled-contenteditable capture/projection/selection binding, composing `createEditorHistory` and `applyOp` directly (#2109). `translateBeforeInput` is a pure, doc-free `beforeinput` -> op translation (`insertText`, else `null`); deletes, merges, and paste are doc-dependent and owned by `bindEditor`, and every op carries `InlineContent[]` rather than a bare string so `applyOp`'s exact `runsEqual` check holds. `projectDocument` reconciles model -> DOM touching only blocks whose object identity changed (verified against a 1000-block identity gate). `bindEditor` `preventDefault`s `beforeinput` so the browser never mutates the DOM itself, routes every edit through `history.controls`, restores the DOM selection from `state.sel` on each render, and catches `applyOp` throws so a rejected edit can never leave the DOM diverged from the model; IME input commits as one `insertText` on `compositionend`. Delete is grapheme-safe -- `deletionRange` reads the browser's `beforeinput` `targetRanges` (via a new `domOffsetInBlock`, the inverse of `locatePosition`) instead of raw UTF-16 offset arithmetic, so backspacing an astral emoji no longer leaves a lone surrogate -- and a non-collapsed selection spanning two blocks deletes the whole range via `deleteRangeAcrossBlocksOps`, built entirely from the existing op vocabulary. Proven by a Playwright harness (no dev server) across chromium/firefox/webkit.
- feat(editor): the editor's own architecture score, class projection, and three framework decorators (#2107). `editor.behavior.ts` gains the score (`parts`, `editorAria`, `editorKeymap`) alongside `bindEditor` -- hand-written per the editor's spec shape, deliberately outside the behavior-layer `compose()`/`BehaviorSpec` path. `bindEditor` now applies `editorAria` through the aria-manager and wires undo/redo on both the Mac and the Windows/Linux chords, gated on `history.canUndo`/`canRedo`; its signature gains an optional `bindEditor(root, history?)` so React's `Editor` can inject the same memory cell it reads via `useMemory` (without injection the contenteditable would construct its own history and the two would diverge). Adds `editor.classes.ts` (root-only class projection keyed off the `data-disabled`/`data-readonly` attributes actually set) and the `editor.tsx` / `editor.element.ts` / `editor.astro` decorators, each opening with the identical six-tag intelligence JSDoc and delegating to `bindEditor` with no reducer/aria/keymap decision of its own. `document-editor.ts` is migrated off the snapshot-based `history.ts` onto this op-based `editor-history.ts` (a diff-to-ops translation for its DOM-first reconcile path, with a `memory` bypass for wholesale `setBlocks`/`addBlocks` that the op vocabulary cannot express), and `bindEditor` re-parses its `disabled`/`readonly`/`label` config on every render so post-mount config changes no longer read stale. React/WC/Astro conformance suites including the axe tier land with it.
- feat(editor): one authored scenario table drives both the DOM-free and the real-contenteditable editor test layers (#2106). `packages/ui/test/harness/caret.ts` ports the prototype's `parseCaret`/`formatCaret` caret-notation grammar verbatim and its given/when/then drive the REAL `createEditorHistory`/`applyOp`, never a parallel model; `editor-scenarios.ts` is the single named scenario list (type, backspace, delete, replace-selection, paste, undo, redo, multi-step unwind, redo-fork, and the canonical `he[llo]` -> type `y` -> `hey|` -> undo -> `he[llo]` case) that both `editor.behavior.test.ts` (vitest) and `editor-capture.e2e.ts` (Playwright) replay, so the two layers prove the same behavior from one source rather than a re-authored, drifting second set. The Playwright half is filtered to collapsed-caret scenarios -- the harness can only seed a collapsed position and `bindEditor` never reads the live DOM selection back into the model (out of scope per the pinned ruling), so range-seeded scenarios stay proven at the model level. The assertion helper is exported as `thenAssert`, not the literal `then`, because a module exporting a callable `then` is treated as a thenable by Vite's SSR loader and invoked with `(resolve, reject)`, crashing Vitest's collection; call sites alias it via `import { thenAssert as then }`. `parseCaret` throws descriptively on malformed notation (unbalanced/reversed brackets, both marker types present, wrong caret count), each throw path now covered by a test so a broken scenario fails loudly at authoring time.

## 0.0.85

### Features

- feat(mcp): `rafters_generate` resolves a prose query to ONE component's verbatim, target-correct snippet with open content slots (#2114). Replaces the honest `{ implemented: false }` stub. A light, deterministic direct-name tier runs first: `normalizeGenerateQuery` lowercases, trims, strips at most one leading filler phrase ("give me a button" -> `button`, "a container" -> `container`), then strips trailing punctuation and compares for exact equality against a real `component`-kind graph node id -- no fuzzy match, no alias table, just the graph's own ids. On a miss it falls back to the same intent door `rafters_describe` uses (`matchIntent`, called with the ORIGINAL unmodified query so filler-stripping never touches curated-axis keyword matching). Whichever tier wins, the id is re-resolved through `describe(id, graph, target)` so the response carries the workspace target's facet, and returns `{ component, target, snippet, slots }` -- `snippet` byte-identical to the registry facet, one `{ slot, ownedBy: 'caller', status: 'open' }` per facet slot in order (`[]` when none). Composites are refused: one reached via the intent door is named ("resolved, but is a composite"), a composite id typed directly falls through to the generic flat refusal. No configured target, or no facet for the target, each return a structured error naming the component (and target). No parameterization, no composition, no writes, no MCP-side validation -- generate is describe's sibling verb serving the same registry facts in a paste-ready shape.
- feat(editor): an invertible `EditorOp` vocabulary (`StructuralOp | FormatOp | TextOp`) with `applyOp` / `applyOpSequence` dispatch -- the foundation the document editor's undo/redo history builds on (#2105). Every op derives its own inverse as an `EditorOp[]` sequence, so a forward op followed by its inverse restores the document to a byte-exact prior state. Structural ops (split/mergePrev/mergeNext/delete/convert/insert) wrap the existing `block-operations` primitives; a merge's inverse is a `[removeText, insert]` pair, because no single op can both truncate the survivor block and reinstate the absorbed one with its original type, meta, and content. Format ops (applyMark/removeMark) splice `InlineContent[]` directly and derive one inverse op per disjoint marked/unmarked sub-range, so undoing a mark over a partially-marked range restores it exactly -- `href` included, so re-linking already-linked text with a new href no longer drops the old one on undo, and two adjacent links carrying different hrefs no longer collapse into one on undo. Text ops (insertText/removeText) are new primitives; `removeText` self-verifies the content it expects before removing, and a merge's `removeText`-based inverse canonicalizes the absorbed content before that equality check, so a raw pasted/deserialized run-array no longer throws and kills the undo stack. `block-operations` is fixed in place to take caller-assigned block ids (so redo replays with stable ids) and to preserve `InlineContent[]` marks across every split/merge point instead of coercing content through `blockContentToText`. The run-slicing helpers in `ops/content.ts` now delegate to those primitives rather than carrying duplicate copies -- the drift between the two copies was the root cause of the merge-inverse crash.

### Bug Fixes

- fix(mcp): guard `overlayContext` against a malformed `installed` block in `.rafters/config.rafters.json` (#2117). A config that parses as valid JSON but shapes `installed.components`/`primitives`/`composites` as anything other than a string array (a bare string, number, or object) previously crashed with an uncaught `TypeError` three calls downstream -- `new Set(config.installed.components)` in `buildInstalledSet` and the primitives spread both throw on a non-iterable. `overlayContext` now validates those three fields once through a Zod schema before any downstream read and returns a discriminated union; `rafters_describe`, `rafters_generate`, and the deprecated `rafters_component`/`rafters_composite` aliases each narrow on it and return a structured `{ error }` naming the config path and the offending field (e.g. `malformed config at /project/.rafters/config.rafters.json: installed.components Expected array, received string`) instead of throwing. The union return type makes the guard structural -- a future call site that forgets to narrow fails to typecheck. Also closes the silent `installed.components: "button"` case, which used to expand a bare string into per-character `Set` entries. A missing config or invalid-JSON-syntax config still degrades to nothing-installed via `readConfig` (unchanged); `installed.rules`/`installed.substrate` are not validated (never read here).
- fix(design-tokens): Tailwind exporter now registers `--leading-*` as a NAMED Tailwind v4 theme variable in `@theme`, bridged from the existing `line-height-*` scale tokens (#2065). `--line-height-*` is not a Tailwind v4 theme namespace, so `leading-xs`, `leading-relaxed`, etc. built from the rafters type scale compiled to nothing even though the value existed in the sheet -- the same silent-drop shape as the `duration-*` trap (#1955). Both `@theme` assembly paths (the live consumer sheet and the Studio static sheet) emit the bridge, and the documentation-sheet candidate deriver now treats `leading-*` as a direct utility name (same as `ease-*`/`animate-*`). `--line-height-*` is additive, not replaced -- existing consumers of that name are unaffected.
- fix(design-tokens): `regenerateOutputs` deletes a stale artifact when its export is disabled (#1944). Turning an export off left its previously-written file on disk, so a consumer kept reading an artifact the config no longer produced. Regen now unlinks the disabled export's output -- best-effort, so a failed unlink (already gone, permissions) never fails the regeneration itself.
- fix(design-tokens): pass-2 token binding resolves in dependency order, not array order (#1634). The second binding pass walked tokens in the order they appeared in the array, so a token whose value reads a dependency defined later in that array bound against the dependency's stale, pre-resolved value. Binding now toposorts by dependency edges before pass 2, so every token reads its dependencies' final values regardless of authoring order.
- fix(design-tokens): a failed cascade rolls back instead of leaving partial state (#1626). `cascadeFrom` applied a chain of dependent updates with no failure boundary, so a plugin throwing mid-cascade left the token graph half-updated. It is now wrapped in try/catch with `undo()` on error, so a mid-cascade failure restores the pre-cascade state atomically -- the whole cascade lands or none of it does.
...
- feat(mcp): `rafters_describe` now returns the whole node -- the complete encoded judgment and how to build the piece in your target (#2090). `assembleGraph` carries the full `intelligence` onto every node (cognitive load, semantic meaning, accessibility, attention economics, trust building, do/never -- previously three of the six were silently dropped) and all of a node's per-target facets from #2073. The graph stays universal and target-free; the workspace's one target is the reader's lens, which `describe(addr, graph, target)` applies to resolve the prop surface (literal-union values, structured constraints, defaults, deprecations) plus the target-correct snippet/slots. A grammar prop (`fill`) drills to its composition rules (`grammar`, `onInvalid`, `default`) so an agent can form a value; its token vocabulary is never surfaced and `describe(<id>.props.<name>.vocab)` is unreachable (#2094), because the fill system manages role words in the managed prop and a bare vocab dump would land in `className` -- the agent composes through the designer's decisions, never around them. Degraded mode (no configured target) advertises no prop children and errors on a prop drill rather than guessing. The stopgap facet side-index is gone -- `rendersForTarget` reads the node's own facets.
...
- feat(design-tokens): each animated matrix cell drives its own animation, and keyframes carry only geometry (#2017). Six new `animate-<component>-<part>-<transition>` utilities -- dialog, popover and dropdown-menu content, open and close -- each emitted as animation longhand referencing the tier and curve its row of `motion.jsonl` assigns. They replace a single shared `animate-scale-in` / `animate-scale-out` that stood in for three distinct moments at one baked duration and curve, so a popover now arrives a tier quicker than a dialog and a dialog leaves a tier slower than a popover, as the matrix has always said. The `scale-in` / `scale-out` keyframe bodies stop carrying a ratio-derived number (`1/ratio^0.25`) and reference `var(--rafters-extent-pop)` instead, giving the extent namespace its first consumer: the entrance scale now has exactly one home and Studio can retune it. Nothing in the new utilities is a literal, so retuning a duration or curve leaf moves one line of the emitted sheet and every cell that references it follows. Reduced motion is handled inside the generated utility by zeroing `animation-duration`, which preserves the keyframe's end state and still fires `animationend` -- `motion-reduce:animate-none` is removed from the three consuming components, because `animation: none` would reset the shorthand and discard both. The new utilities are emitted into both sheets -- the consumer one and Studio's static one -- and an operator pin (`rafters set motion-cell-<cell> "<shorthand>"`) emits as the shorthand it is, with the reduced-motion block still attached. The other components with assigned content cells in the matrix (alert-dialog, context-menu, tooltip, hover-card, select, combobox, command, menubar, navigation-menu) consume no animation today and are unchanged; converting them is the component sweep.

### Bug Fixes

- fix(cli): `add --update-all` no longer leaves an untracked component stale forever, and says what it actually did (#2018). The candidate list came from `config.installed` alone, so a component sitting on disk that the config never recorded was invisible to every future update -- no code path discovered it. The reproduction is quiet and total: remove a component from `installed`, delete one of its files, run `--update-all`, get a success report and a missing file, fixed only by naming the component. `--update-all` now reconciles before it runs -- it asks the registry index what exists and the project which of those names are present, then updates the union of that and the tracked set, so an untracked component is refreshed and tracked on the next run. The scan is index-first, so consumer-authored files in the same folders are never mistaken for registry items, and an unreachable index degrades to updating the tracked set with a warning rather than aborting. Each candidate still resolves through `resolveDependencies`, so updating a parent re-walks its closure and re-tracks any dependency the config had lost. `getInstalledNames` now includes the `rules` and `substrate` buckets it was silently dropping. The summary is the other half: `Added N components` over a mixed-state tree is what made this cost hours, so the report now separates written, skipped, untracked-and-now-tracked, and failed, in both agent JSON (`written` / `skipped` / `untracked` / `failed` with their name lists) and human output. Agent consumers reading `installed` off `add:complete` should read `written`. The top-level signal is now honest too: the human headline said "Wrote N items" under a green tick even when every item failed, and the process still exited 0 -- honest per-outcome lines beneath a success mark are the same lie one level up, and a scripted install reading only the exit code called a half-written tree done. A run with failures now renders a failure headline and exits 1.
- fix(cli): `rafters --version` reports the version the binary actually is (#2021). It printed a hardcoded `0.0.1` for every build ever published, which is exactly backwards: `--version` is the first thing anyone reaches for when a `pnpm dlx rafters@latest` resolves from a stale metadata cache, and this org has already lost hours to that failure once. The version is now read at runtime from the package's own `package.json`, which resolves identically from `src/` and from the bundled `dist/`, so the test that asserts the spawned CLI prints `packages/cli/package.json`'s version covers what consumers actually execute -- a build-time inject would leave the source path (the path vitest runs) reporting a placeholder. The MCP server handshake, which advertised the same `0.0.1`, now reports the real version too. Printing the resolved registry URL alongside it was considered and deliberately left out: resolving it means loading project config from a global flag that does not currently touch the project.
- fix(design-tokens): every `animate-*` utility now carries a duration and a timing function (#2000). Generated animation values were built on `var(--motion-duration-<tier>)` and `var(--motion-easing-<curve>)`, and nothing has ever declared either name in either emission path. A dangling `var()` inside an animation shorthand does not fail: `animation: scale-out  ;` parses, computes to a zero duration, and the animation silently never runs -- green build, green tests, no motion. The values now reference the namespace leaves the sheet actually declares (`--rafters-duration-*`, `--rafters-ease-*`). The static Studio sheet had a second copy of the same bug, bridging `--animate-*` onto an invented `--rafters-animate-*` namespace no token is named for; it now emits the animation value directly, exactly as the dynamic path does, and retheming still reaches it because the duration and easing inside that value are themselves references to the declared leaves. Emitted animation lines change; the dangling-var guard now sweeps these prefixes, so a reintroduction fails the suite rather than shipping.
