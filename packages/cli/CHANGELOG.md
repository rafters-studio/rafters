...
- refactor(design-tokens, ui): every hand-rolled OKLCH-to-CSS-string formatter in `packages/design-tokens` and `packages/ui` now delegates to `oklchToCSS` from `@rafters/color-utils` (#2147), so `rafters add`'s delivered source for `color-swatch`, `color-scale`, `cvd-simulation`, `color-area`, and `hue-bar` changes: their `oklch()` string formatting now goes through `oklchToCSS` instead of a local template literal. `color-swatch.ts`'s alpha-aware `toOklch` is byte-identical (alpha now flows through `oklchToCSS` automatically). The four plain-formatting `packages/ui` sites (`color-scale`, `cvd-simulation`, `color-area`, `hue-bar` x2) and `design-tokens`'s `dtcg.ts`/`generators/color.ts` sites are byte-identical by construction -- none of them rounded before, and `oklchToCSS` with no `precision` option doesn't either. `design-tokens`'s `tailwind.ts` site drops its local `formatNumber` (round to 3 decimals, then strip trailing zeros) for `oklchToCSS` with no precision option; every OKLCH value reaching that exporter is already rounded to <=3 decimals upstream, so the two are byte-identical for every case the fixture tests and the full generated base system exercise. `typescript.ts`'s site is NOT fully byte-identical: it previously formatted L/C to 3 decimals and H to 1 decimal separately, and `oklchToCSS`'s single `precision` option cannot reproduce mixed per-channel precision, so H gains trailing zeros (`"180.0"` -> `"180.000"`) -- the color value is unchanged, only the string's trailing zeros differ. Each of the five changed `packages/ui` primitives now carries an `@internal-dependencies @rafters/color-utils` JSDoc tag.
