...
Intent: `feedback-loop`. The shimmer is `animate-pulse`; duration and easing come
from tokens. The pulse opts out under `prefers-reduced-motion` via
...
| --- | --- |
| `animate-pulse` shimmer | contract |
| `motion-reduce:animate-none` reduced-motion opt-out | contract |
...
- 1.4.13 Animation from interactions / 2.3.3: the pulse honours
