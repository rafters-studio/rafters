...
| **bar** | One data series for a BarChart (compositional child, never a prop) | Carries `dataKey`; BarChart derives its series list from composed `<Bar>` children, in order, taking precedence over its own `series` config when at least one is present (#2225) | - | - | - | none | none | react, wc, astro |
| **bar-chart** | Grouped/stacked bar marks, shadcn-API-compatible (#2225) | Computes bar geometry from data via bandScale/linearScale (computeBars); resolves series color via bar-chart.classes.ts resolveBarFillClass (chart.classes.ts resolveSeriesClass under the hood); keyboard traversal (arrows/Home/End) moves an active-datum cursor announced via sr-announcer; pins the family's accessible structure (figure + aria-hidden SVG + always-present data-table fallback) | bars, activeIndex (active-datum cursor) | graph.ts bandScale/linearScale/ticks, sr-announcer | - | two layout-specific matrix cells (bar-chart/bar/enter vertical, bar-chart/bar/enter-horizontal, motion.jsonl; BehaviorSpec.motion is spec-reserved but unimplemented, #1990 open) | none | react, wc, astro |
