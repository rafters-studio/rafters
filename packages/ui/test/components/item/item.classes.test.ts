...
describe('item classes', () => {
  ...
  it('resolves each size to its padding + typography role token', () => {
    ...
    expect(itemSizeClasses.default).toBe('px-3 py-2 ts-body-small');
    expect(itemSizeClasses.sm).toBe('px-2 py-1.5 ts-label-small');
    expect(itemSizeClasses.lg).toBe('px-4 py-3 ts-body-medium');
  });
  ...
  it('sub-part classes are config-independent literals', () => {
    ...
    expect(itemDescriptionClasses).toBe('truncate text-muted-foreground ts-label-small mt-0.5');
  });
