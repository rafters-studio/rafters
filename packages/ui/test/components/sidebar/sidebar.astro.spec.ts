 ...
 * and a11y tree.
 *
 * This file also carries the Astro sub-component DROP-IN PARITY SURFACE: an
 * Astro tree has to compose exactly like the React tree --
 *
 ...
 *
 * The final describe block exercises `injectAsChildAttrs`
 * (src/primitives/astro-as-child.ts) directly -- the render-then-inject
 * primitive every Astro `asChild` sub-component above depends on. It is
 * colocated here (not in a shared test-support module) because sidebar's
 * asChild surface is its only exerciser in this trim.
 */
