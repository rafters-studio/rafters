...
test('role=grid: honest role projected, name and columns wired', async () => {
...
test('role=grid: arrow keys rove in two dimensions', async () => {
  ...
  pressKey(grid[0] as HTMLElement, 'End');
  expect(document.activeElement).toBe(grid[3]);
  pressKey(grid[3] as HTMLElement, 'Home');
  expect(document.activeElement).toBe(grid[0]);
});

test('presentation grid stays inert: no role, no roving, priority preserved', async () => {
