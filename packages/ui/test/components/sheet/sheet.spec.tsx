...
function domPartIds(root: HTMLElement, parts: readonly SheetPart[]): Record<SheetPart, string> {
  const ids = {} as Record<SheetPart, string>;
...
describe('sheet [react]', () => {
  ...
  it('open: every part renders and ARIA equals the projection', async () => {
    ...
    const allParts = Object.keys(sheet.parts) as SheetPart[];
    const ids = domPartIds(body(), allParts);
    const projection = sheet.aria(state, config, ids);

    for (const part of expectedParts) {
      const element = partElement(body(), part);
      expect(element, `declared part "${part}" must be rendered`).not.toBeNull();
      const decl = sheet.parts[part];
      if (decl.role) {
  ...
  it('trigger and content are wired by real DOM ids', async () => {
    ...
    render(<TestSheet />);
    await user.click(partElement(body(), 'trigger') as HTMLElement);
    const ids = domPartIds(body(), ['trigger', 'content', 'title', 'description']);
    const trigger = partElement(body(), 'trigger');
    const content = partElement(body(), 'content');
