 ...
 * through) holds in the shadow-DOM performance too. `kbd.element.ts`
 * self-registers `rafters-kbd` on import, guarded, so no explicit
 * customElements.define is needed here.
 */
import { afterEach, describe, expect, it } from 'vitest';
import { RaftersKbd } from '../../../src/components/kbd/kbd.element';

function mount(slots = ''): HTMLElement {
  document.body.innerHTML = `<rafters-kbd>${slots}</rafters-kbd>`;
...
describe('kbd [wc]', () => {
  ...
  it('root projects NO ARIA (empty, like React)', () => {
