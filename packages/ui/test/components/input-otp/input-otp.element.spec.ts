...
describe('input-otp [wc]', () => {
  it('empty: the projection matches the rendered DOM', async () => {
    const root = await mount();
    assertContract(root, { value: '', activeIndex: 0 }, { maxLength: 6 }, [
      'root',
      'input',
      'group',
      'slot',
  ...
  it('backspace deletes through the native field and repaints the slots', async () => {
    ...
    const el = input();
    el.focus();
    // A real browser's programmatic focus() does not put the caret at the
    // end the way happy-dom's did; place it explicitly so Backspace deletes
    // the last character, matching the field's actual typing behavior.
