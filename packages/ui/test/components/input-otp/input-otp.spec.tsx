/**
 * Browser-project performance of the input-otp score, driven end to end. The
 * oracle's compound surface (InputOTP + Group/Slot/Separator) over one real
 * input.
 */
...
function assertContract(
  ...
  expectedParts: ReadonlyArray<InputOtpPart>,
): void {
  for (const part of expectedParts) {
    expect(partElement(root, part), `declared part "${part}" must be rendered`).not.toBeNull();
  }
  const allParts = Object.keys(inputOtpBehavior.parts) as InputOtpPart[];
  const ids = {} as Record<InputOtpPart, string>;
  ...
  for (const slot of partElements(root, 'slot')) {
    const value = slot.dataset['value'];
    if (value === undefined) continue;
    const attrs = inputOtpBehavior.instanceAria?.('slot', value, state, config, ids) ?? {};
    for (const [attr, projected] of Object.entries(attrs)) {
      if (projected === undefined) {
        expect(slot.hasAttribute(attr), `slot "${value}" must NOT render ${attr}`).toBe(false);
      } else {
        expect(slot.getAttribute(attr)).toBe(String(projected));
      }
    }
  }
}
...
describe('input-otp [react]', () => {
  it('empty: every declared part renders and the projection matches', async () => {
    const { container } = await render(<Field />);
    assertContract(container, { value: '', activeIndex: 0 }, { maxLength: 6 }, [
      'root',
      'input',
      'group',
      'slot',
  ...
  it('names the field by its slot count and offers one-time-code autofill', async () => {
    const { container } = await render(<Field />);
  ...
  it('the slots are the only visible surface: no extra tab stops', async () => {
    const { container } = await render(<Field />);
  ...
  it('refuses characters the pattern rejects -- they never reach a slot', async () => {
    ...
    const { container } = await render(<Field onChange={onChange} />);
  ...
  it('a custom pattern replaces the digit rule', async () => {
    const user = userEvent.setup();
    const { container } = await render(<Field pattern={/^[a-z]$/} />);
    await user.type(inputOf(container), 'a1b');
  ...
  it('backspace walks back through the code', async () => {
    const user = userEvent.setup();
    const { container } = await render(<Field defaultValue="123" />);
    const el = inputOf(container);
    el.focus();
  ...
  it('paste splits a whole code across the slots, separators and all', async () => {
    ...
    const { container } = await render(<Field onComplete={onComplete} />);
    const el = inputOf(container);
    el.focus();
  ...
  it('completion fires once on the edge, and the root reports it', async () => {
    const user = userEvent.setup();
    const onComplete = vi.fn();
    const { container } = await render(<Field defaultValue="12345" onComplete={onComplete} />);
    const el = inputOf(container);
    el.focus();
  ...
  it('a complete code keeps the last slot lit after ArrowLeft (the oracle rule)', async () => {
    const user = userEvent.setup();
    const { container } = await render(<Field defaultValue="123456" />);
    inputOf(container).focus();
  ...
  it('controlled: the value follows the prop, the callback reports the intended edit', async () => {
    const user = userEvent.setup();
    const onChange = vi.fn();
    const { container, rerender } = await render(<Field value="12" onChange={onChange} />);
    const el = inputOf(container);
    expect(el.value).toBe('12');
    await user.type(el, '3');
    expect(onChange).toHaveBeenLastCalledWith('123');
    expect(el.value).toBe('12');
    await rerender(<Field value="99" onChange={onChange} />);
  ...
  it('disabled gates every entry path and projects the state', async () => {
    const user = userEvent.setup();
    const onChange = vi.fn();
    const { container } = await render(<Field disabled onChange={onChange} />);
    const el = inputOf(container);
  ...
  it('required and name make it a real form field, no hidden mirror', async () => {
    const { container } = await render(<Field required name="code" defaultValue="123456" />);
    const el = inputOf(container);
