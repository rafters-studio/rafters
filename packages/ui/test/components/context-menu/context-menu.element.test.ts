 ...
 *
 * Its assertions are focus and popover state after a gesture, and that is the
 * exact shape of an open Vitest defect: browser mode does not reset the
 * browser between test FILES, and popover open/close plus
 * `document.activeElement` sit on task queues nothing awaits across suites
 * (vitest-dev/vitest#5706, pending triage). `isolate: true` does not help, as
 * it isolates the iframe while focus lives on the page above it. Measured
 * here on the ported file: passes 4 runs of 4 alone, fails roughly every
 * other full-suite run, always these same focus assertions.
 *
 * Four fixes were tried against it and none worked: running with
 * `--no-file-parallelism`, converting 27 assertions to `expect.poll`,
 * flushing deferred work in `afterEach` before clearing the body, and setting
 * a desktop viewport instead of Vitest's 414x896 default. The failure is
 * upstream, not in this file.
 *
 * happy-dom has no such problem, because `document.activeElement` there is a
 * plain property with no real focus behind it, and each file gets a fresh
 * document. Move this back to `.spec` when #5706 lands.
 */
...
describe('context-menu [wc]', () => {
  ...
  // layout (the CSS reveal and transition are untouched by it). Roving-focus
  // leaves one item at `tabindex="0"` when it tears down, which would be a
  // focusable descendant of an aria-hidden container unless that tabindex is
  // also reset.
  it('closed sub-content is aria-hidden with every item back at tabindex="-1" after an open/close cycle', async () => {
    const user = userEvent.setup();
    await mount();
    fireEvent.contextMenu(trigger(), { clientX: 10, clientY: 10 });
    const st = document.getElementById('cm-sub-trigger') as HTMLElement;
