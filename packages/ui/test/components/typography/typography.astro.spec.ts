/**
 * Astro performances of the Typography score: the generic `<Typography as>`
 * wrapper, AND the drop-in surface of 20 named exports typography.tsx also
 * exports -- served from the SAME single typography.astro file as hoisted
 * createComponent exports next to the default export (#2325).
 *
 ...
 * whitespace differs across majors. The named-exports half parses with
 * happy-dom's own Window instead of the test environment's globals, because
 * from Astro 6 the Container API renders only under vitest `environment: 'node'`.
 */
...
function partElement(scope: ParentNode, part: string): HTMLElement | null {
  return scope.querySelector<HTMLElement>(`[data-part="${part}"]`);
}
