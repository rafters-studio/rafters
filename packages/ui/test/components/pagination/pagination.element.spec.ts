 ...
 * React and Astro performances -- but Pagination is a PURE STATIC, so there is
 ...
 * label, slotted content projects through with its earned semantics) holds in
 * the shadow-DOM performance too. Axe cleanliness for this markup is asserted
 * in the .a11y tier, not here.
 */
...
describe('pagination [wc]', () => {
  ...
  it('fulfills the contract: root projects NO ARIA role (empty, like React)', () => {
    ...
    expect(pagination.aria({}, {}, { root: root.id }).root).toEqual({});
