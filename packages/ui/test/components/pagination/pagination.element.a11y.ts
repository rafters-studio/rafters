...
import '../../../src/components/pagination/pagination.element';

// The element owns only the nav landmark; the trail is consumer-composed light
// DOM slotted through. These are the trails a consumer would slot in.
const ELLIPSIS =
...
function mount(trail: string): HTMLElement {
  document.body.innerHTML = `<main><rafters-pagination>${trail}</rafters-pagination></main>`;
...
for (const [name, trail] of scenes) {
  test(`rafters-pagination ${name}`, async ({ task }) => {
