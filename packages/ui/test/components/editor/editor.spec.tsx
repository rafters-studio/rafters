 ...
 *
 * The editor is deliberately NOT a compose()/BehaviorSpec component
 * (RULING-EDITOR-HISTORY, frozen Spec 00 line 132) -- `editorAria`/`parts` are
 * hand-written, not bundled into that shape. Assertions below compare the
 * rendered DOM against `editorAria`'s own projection entry-by-entry instead,
 * including the undefined -> absent case.
 */
...
describe('editor [react]', () => {
  ...
  it('undo/redo gate: Cmd+Z before any edit is a silent no-op', () => {
    ...
    // proven in editor.element.spec.ts and editor.astro.spec.ts, which seed a
    // real block via data-initial-doc. React's own initialDocument-seeded
    // equivalent lives in editor.react-props.test.tsx (#2212) alongside the
    // rest of that prop's coverage, not here. Same bindEditor, same
    // editorKeymap, same history.canUndo/canRedo gate in every performance --
    // proven once at the DOM-native layer covers all three.
  });
