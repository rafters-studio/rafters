/**
 * React performance of the resizable score, driven end to end, plus the
 * compound-only onLayout proof. Every scenario in SCENARIOS is asserted for
 * structure/aria (parts + role=separator handles) and for the keymap
 * (Arrow/Shift+Arrow/Home/End resize the leading panel).
 */
...
function renderScenario(props: ResizableScenarioProps, label: string) {
  ...
  return { root, cleanup: () => utils.unmount() };
}

describe('resizable [react]', () => {
  for (const scenario of SCENARIOS) {
    it(`${scenario.name}: parts and aria match the behavior projection`, () => {
      const { root, cleanup } = renderScenario(scenario.props, 'Resize section');
      try {
    ...
    it(`${scenario.name}: one fewer separator than panels, each role=separator`, () => {
      const { root, cleanup } = renderScenario(scenario.props, 'Resize section');
      try {
  ...
  it('the first handle reports the leading panel size as aria-valuenow', () => {
    const { root, cleanup } = renderScenario(
