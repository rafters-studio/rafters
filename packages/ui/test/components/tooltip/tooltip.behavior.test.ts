...
describe('tooltip aria projection', () => {
  it('closed: STILL described-by the content, and never expanded', () => {
    // UNCONDITIONAL (#2148). The tip is present in the DOM at all times and the
    // reveal is CSS, so gating the description on `open` would mean the trigger
    // has no description during the frames JavaScript considers it closed --
    // and, on a JS-off page, never.
