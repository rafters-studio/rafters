...
/**
 * Markup/behavior pairing #2001: config reaches a DOM-native binding as
 * `data-*` and nothing else. Assert BOTH halves on the root -- the dataset
 * value present, AND the bare pre-fix spelling (`sideOffset` -> `side-offset`)
 * absent -- so a half-fix (markup renamed, behavior still on getAttribute, or
 * the reverse) fails loudly instead of rendering fine and silently not
 * rehydrating.
 */
