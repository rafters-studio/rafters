 ...
 * configures a delay.
 *
 * WHY THIS ONE KEEPS THE `.test` NAME (#2329). Its assertions are dismissal
 * state after a pointer leave and a blur, which is focus behaviour, and that
 * is the shape vitest-dev/vitest#5706 breaks: browser mode does not reset the
 * browser between test FILES, so popover state and `document.activeElement`
 * carry across suites. Measured on the ported file with the browser lane's
 * memory bounded (peak 0.98GB, so not starvation): passes 3 runs of 3 alone
 * and 3 of 3 beside its React sibling, and fails intermittently only in the
 * full 246-file run, always these same two dismissal tests.
 *
 * Same call as context-menu.element.test.ts, and the same escape: happy-dom
 * gives each file a fresh document and has no real focus behind
 * `activeElement`. Move both back to `.spec` when #5706 lands.
 */
