 ...
 * half lives in navigation-menu.classes.ts and test/motion/hover-reveal.spec.ts.
 */
 ...
 *  per-instance projection, sibling ids resolved from the DOM. */
function assertNavInstanceAria(
  root: HTMLElement,
  state: NavigationMenuState,
  config: NavigationMenuConfig,
): void {
  const manyParts: NavigationMenuPart[] = ['trigger', 'content'];
  for (const part of manyParts) {
    for (const element of Array.from(root.querySelectorAll<HTMLElement>(`[data-part="${part}"]`))) {
      const value = element.dataset['value'];
      if (value === undefined) continue;
      const ids: Partial<Record<NavigationMenuPart, string>> = {};
      for (const sibling of manyParts) {
        ids[sibling] =
          root.querySelector<HTMLElement>(`[data-part="${sibling}"][data-value="${value}"]`)?.id ??
          '';
      }
      for (const [attr, projected] of Object.entries(
        navInstanceAria(part, value, state, config, ids),
      )) {
...
describe('navigation-menu [wc]', () => {
  ...
  it('per-instance ARIA equals the score projection, closed and open', async () => {
    ...
    assertNavInstanceAria(root, { active: null, pointerOpened: false }, {});
    await user.click(trigger('products'));
    assertNavInstanceAria(root, { active: 'products', pointerOpened: false }, {});
  });
