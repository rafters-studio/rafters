 ...
 * per-row selected projection holds. One score, two performances (React +
 * Astro); here the performance is markup + classes + slot, nothing more.
 */
...
describe('table conformance [astro]', () => {
  ...
  it('a selected row projects aria-selected and data-state; header/unselected project neither', async () => {
    const dom = await render();
    const root = partElement(dom, 'root') as HTMLElement;
    assertRowContractFulfillment(root, ['header', 'ada', 'grace'], (key) =>
