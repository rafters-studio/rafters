...
import { bindSlider, sliderBehavior } from '../../../src/components/slider/slider.behavior';
...
describe('slider [astro]', () => {
  ...
  // resolver computed actually reaches the SSR output.
  for (const [name, props] of [
    ['default', {}],
    ['small', { size: 'sm' }],
    ['destructive lg', { variant: 'destructive', size: 'lg' }],
    ['disabled', { disabled: true }],
    ['vertical', { orientation: 'vertical' }],
  ] as const) {
    it(`${name}: the SSR markup carries the resolved class string on every part`, async () => {
      const root = await mount(props as Record<string, unknown>);
      const config = {
        variant: 'default',
        size: 'default',
        min: 0,
        ...
        disabled: false,
        ...props,
      } as Parameters<typeof sliderClasses>[0];
      const classes = sliderClasses(config, sliderBehavior.initialState(config));
      expect(root.className).toBe(classes.root);
      expect((root.querySelector('[data-part="track"]') as HTMLElement).className).toBe(
        classes.track,
      );
      expect((root.querySelector('[data-part="range"]') as HTMLElement).className).toBe(
        classes.range,
      );
      for (const thumb of thumbs(root)) expect(thumb.className).toBe(classes.thumb);
    });
  }
