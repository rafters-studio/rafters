 ...
 *   <rafters-area data-part="series" data-key="mobile" hidden></rafters-area>
...
/**
 * WC performance of the area score. Pure static with an empty projection
 * (like x-axis.element.ts / bar.element.ts): no bind, no controller -- the
 * element is a light-DOM marker the owning AreaChart's bind later reads via
 ...
 * computed here.
 */
export class RaftersArea extends HTMLElement {}

if (typeof customElements !== 'undefined' && !customElements.get('rafters-area')) {
  customElements.define('rafters-area', RaftersArea);
}
