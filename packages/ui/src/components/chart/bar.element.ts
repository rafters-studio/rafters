 ...
 *   <rafters-bar data-part="series" data-key="mobile" hidden></rafters-bar>
...
/**
 * WC performance of the bar score. Pure static with an empty projection
 * (like x-axis.element.ts): no bind, no controller -- the element is a
 * light-DOM marker the owning BarChart's bind later reads via
 ...
 * computed here.
 */
export class RaftersBar extends HTMLElement {}

if (typeof customElements !== 'undefined' && !customElements.get('rafters-bar')) {
  customElements.define('rafters-bar', RaftersBar);
}
